...
| Setting | Default | Description |
|---------|---------|-------------|
| `enabled` | `true` | Master switch for regime-scaled position sizing. When false, all entries use fixed `strategy.entryBudgetSol`. |
| `bullMultiplier` | `1.5` | Size multiplier when market regime is bull (larger positions in uptrends). |
| `consolidationMultiplier` | `1.0` | Size multiplier when market regime is consolidation (normal size). |
| `recoveryMultiplier` | `0.3` | Size multiplier when market regime is recovery (smaller positions — higher risk). |
| `dumpMultiplier` | `0.0` | Size multiplier when market regime is dump (no entries — risk too high). |

**Validation:** Regime-scaled sizing shows +20.2% P&L improvement and 70% loss reduction in recovery regimes vs. fixed sizing.

**How to use:** Enable by default. The scanner reads the current market regime from `data/regime-state.json` (generated by Tier 1 analysis on every reflect cycle) and scales each buy accordingly. If regime detection is off or no regime data is available, sizing falls back to 1.0× (normal). 

---

## Webhooks (Fire-and-Forget Event Dispatch)
...
| Setting | Default | Description |
|---------|---------|-------------|
| `enabled` | `false` | Master switch for outbound webhooks. When false, no events are dispatched. |
| `urls` | `[]` | Array of webhook URLs. Supports HTTP/HTTPS and Discord webhooks. Dead URLs are logged but don't block other endpoints. |
| `events` | `["trade_opened", "trade_closed", "daily_brief"]` | List of event types to dispatch. Valid events: `trade_opened` (position entry), `trade_closed` (position exit), `daily_brief` (end-of-day summary). |

**How to use:** Add webhook URLs from your notification service (Slack, Discord, or custom webhook receiver). Each event fires a 5-second async POST with the trade/summary payload. Failures are logged but fail-open — they don't block trading or other notifications.

**Webhook payload shapes:**
- `trade_opened`: `{ event, mint, entryPrice, entrySol, score, pattern, timestamp }`
- `trade_closed`: `{ event, mint, entryPrice, exitPrice, pnlSol, pnlPct, exitReason, holdMs, timestamp }`
- `daily_brief`: `{ event, wins, losses, pnlSol, winRate, timestamp }`

---

### Tier 10: Dashboard Intelligence Tab

Dashboard tab exposing Tier 2, 3, and 7 backends for operator oversight. No configuration needed — Intelligence tab appears automatically when the agent starts.

**Three sections:**

| Section | Source | What it shows |
|---------|--------|---------------|
| **Approval Queue** | `/api/recommendations/summary` | Pending config change recommendations (Tier 2). Review, approve, or reject each with optional notes. Shows decision counts + pending list. |
| **Skill Performance** | `/api/skills/performance` | Which loaded skills correlate with wins (Tier 3). Shows skill name, avg win rate, avg P&L, performance score, days active, and KEEP/MONITOR/DISABLE recommendation. |
| **Ecosystem Health** | `/api/ecosystem/status` | Network health gauge (Tier 3 + 1). Shows 0–100 health score, breakdown (TPS, validators, MEV, priority fees), and active anomalies (low TPS alert, etc.). |

**Overview tab updates:**
- Regime field now shows current market regime and confidence (e.g., "bull (87%)") instead of `——`. Reads from `data/regime-state.json` (Tier 1/7 data).

**How to use:** Open dashboard → Intelligence tab. Approvals queue updates every reflect cycle (4h). Skill grades update every 4h (when trades match skill usage windows). Ecosystem health updates every 5 minutes (heartbeat cycle). Empty states are graceful — if ecosystem gating is disabled or no skill data yet, sections show "no data available" message.
