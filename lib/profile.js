...
function refresh() {
  ...
  // Keep profile.model in sync with live config so peer agents see the correct model
  const currentModel = loadConfig().llm?.model;
  if (currentModel && profile.model) {
    profile.model = { ...profile.model, primary: currentModel };
  }
...
async function publish(api, { paperMode = false } = {}) {
  const profile = loadProfile();
  if (!profile) return;
  ...
  try {
    await api.swarmPublish({
      ...
      type:       'agent_profile',
      confidence: 1.0,
      data:       { ...profile, ...(paperMode ? { mode: 'paper' } : { mode: 'live' }) },
      ttlSeconds: 3_600,  // 1h — refreshed each reflect cycle (4h) or heartbeat
    });
    log('info', `Profile published to swarm${paperMode ? ' [paper mode]' : ''}`);
  } catch (err) {
...
async function refreshAndPublish(api, opts = {}) {
  const profile = refresh();
  if (profile) await publish(api, opts);
}
...
// saveProfile is exported so callers (e.g. load_skill recording a used skill)
// write through the same atomic path this module owns, rather than rebuilding
// data/agent-profile.json themselves.
module.exports = { refresh, publish, refreshAndPublish, incrementSessions, buildProfileBlock, loadProfile, saveProfile: _saveProfile, loadIdentity };
