...
function scoreDipReversal(c, cfg) {
  ...
  if (pc5m < 1.5)                          gateFailures.push(`5m bounce weak (${pc5m.toFixed(1)}% < 1.5%)`);
  ...
  // Deep dips on thin pools are death spirals — 10s monitor cannot protect against blow-through.
  // Require $50k liquidity for 1h drops >10% (DEEP-REVERSAL territory).
  if (pc1h < -10 && liq < 50_000)         gateFailures.push(`deep-dip thin pool (1h ${pc1h.toFixed(0)}% liq $${(liq/1000).toFixed(0)}k < $50k)`);
