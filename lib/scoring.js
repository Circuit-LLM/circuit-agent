...
function scoreDipReversal(c, cfg) {
  ...
  // stronger, real bounce before catching a deep dip. Both thresholds are tunable per agent
  // (strategy.minBounce5m / strategy.minBounceDeep5m) to trade entry frequency vs quality;
  // defaults preserve the original 1.5 / 3.0 behavior.
  const minBounce5m = pc1h <= -5
    ? (cfg?.strategy?.minBounceDeep5m ?? 3.0)
    : (cfg?.strategy?.minBounce5m ?? 1.5);
  ...
  // Run-up gate (OPT-IN, default OFF — same collect-first rollout as S2): reject entries where
  // price has already run too far off its recent 20-min low; the bounce is spent and tends to
  // mean-revert. Backtested (experiments/backtest/bt_entryfilter.js, 450 signals, 3 windows):
  // runUp <2% → 62% win / ex-top-2 +17%; runUp >9% → 42% win / ex-top-2 −1.4%; a reject-at-5%
  // gate improved win +3-4pts / avg gross +1-2% while keeping ~2/3 of trades. runUpFromLowPct
  // is captured in entryConditions first; enable via strategy.maxRunUpFromLowPct (suggested 5)
  // once live capture confirms the backtest. Null/missing field never gates.
  const maxRunUp = cfg?.strategy?.maxRunUpFromLowPct ?? null;
  if (maxRunUp != null && Number.isFinite(c.runUpFromLowPct) && c.runUpFromLowPct > maxRunUp)
    gateFailures.push(`already ran +${c.runUpFromLowPct.toFixed(1)}% off the 20m low (> ${maxRunUp}%) — bounce spent`);
