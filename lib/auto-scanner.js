...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  if (regimeMultiplier === 0.0) {
    ...
    // through — otherwise swap.buy() is called with 0 SOL. Jupiter rejects that with
    // "Invalid amount", the candidate is silently discarded, and the optimistic buy_signal
    // published moments earlier is left sitting in the swarm feed as a phantom that peers'
    // consensus and recent-buy filters treat as a real entry. Pausing means no trade.
  ...
  // herd penalty, consensus boost and regime multiplier produced it.
  if (!(finalBudget > 0)) {
