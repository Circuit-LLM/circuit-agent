...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  // Scorer is config-selectable (agent2 momentum test): strategy.scorer='momentum' rides confirmed
  // uptrends; anything else keeps the default dip-reversal scorer (unchanged for the rest of the swarm).
  const scorer = cfg.strategy?.scorer === 'momentum' ? scoreMomentum : scoreDipReversal;
