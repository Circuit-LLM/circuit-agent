...
const { scoreMomentum } = require('./scoring-momentum');
const { scoreFlow } = require('./scoring-flow');
...
/**
 * Select the candidate scorer from config. Opt-in by exact string, so an agent that sets nothing
 * keeps the dip-reversal scorer it has always used — adding a scorer here must never change
 * behaviour for the agents that did not ask for it. Exported so that stays a tested invariant
 * rather than an assumption (see tests/auto-scanner.test.js).
 *   'momentum' — rides confirmed uptrends       (lib/scoring-momentum.js)
 *   'flow'     — trades order-flow accumulation (lib/scoring-flow.js)
 *   anything else, including unset — dip-reversal default (lib/scoring.js)
 * Note 'smartmoney' never reaches here: agent.js starts smart-money-scanner instead of this scanner.
 */
function pickScorer(cfg) {
  return cfg?.strategy?.scorer === 'momentum' ? scoreMomentum
       : cfg?.strategy?.scorer === 'flow'     ? scoreFlow
       : scoreDipReversal;
}
...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  // strategy.ignoreSessionOverrides (agent2 data run): pin the entry gate to config so the LLM
  // agent-loop can't quietly change it mid-experiment. Default off = normal session behaviour.
  const minScanScore     = (s.ignoreSessionOverrides ? s.minScanScore : (session.minScoreOverride ?? s.minScanScore)) ?? 55;
  ...
  if (!isWatchOnly) {
    // Measured GROSS on purpose — this breaker exists to detect a losing streak in the
    // STRATEGY, and on the net basis the ~4.9% round-trip fee at 0.005 SOL makes three
    // losses in a row nearly unavoidable regardless of how the entries performed.
    const cbHistory = positions.getTradeHistory(5, 7).slice(-3);
    if (cbHistory.length >= 3 && cbHistory.every(t => positions.grossPnlPct(t) < 0)) {
      const lastExit = new Date(cbHistory[cbHistory.length - 1].exitTime).getTime();
      if (Date.now() - lastExit < 60 * 60_000) {
        const pcts = cbHistory.map(t => `${positions.grossPnlPct(t).toFixed(1)}%`).join(', ');
        log('warn', `Circuit breaker: 3 consecutive gross losses [${pcts}] — pausing buys for 60min`);
        return;
      }
    }
  }
  ...
  // scanLimit widens the active-universe slice the scorer sees (price-feed enriches ~limit*2
  // tokens ranked by recent txn count). Flow needs the broader slice: accumulation tokens have
  // mid-range activity and rank below the top movers, so the default 30 truncates them out.
  const scanLimit = s.scanLimit ?? 30;
  try {
    let result = await api.scanFree({ limit: scanLimit, minLiquidity });
    ...
    if (!isPaper && candidates.length < 3) {
      ...
      const paid = await api.scan({ limit: scanLimit, minLiquidity, safeOnly });
  ...
  const recentlyTraded = new Set(
    recentTrades
      .filter(t => {
        ...
        // GROSS, for the same reason as the breaker above. On the net basis a typical
        // -1% price move books as ~-6% after fees, which banned the mint for 24h — 59% of
        // every token the swarm touched was quarantined for a loss the strategy never made.
        const pnl = positions.grossPnlPct(t);
  ...
  const scorer = pickScorer(cfg);
  const allScored = filtered.map(c => {
    const result  = scorer(c, cfg);
  ...
  const effectivePatternFilter = (!s.ignoreSessionOverrides && session.patternFilter?.length)
  ...
  if (regimeMultiplier === 0.0) {
    // Dump regime = pause entries. This MUST return — not merely zero the budget and fall
    // through — otherwise swap.buy() is called with 0 SOL. Jupiter rejects that with "Invalid
    // amount", the candidate is silently discarded, and the optimistic buy_signal published
    // moments earlier is left sitting in the swarm feed as a phantom that peers' consensus and
    // recent-buy filters treat as a real entry. Pausing means no trade.
    log('info', `Regime pause (dump): ${best.symbol ?? best.mint.slice(0, 8)} — entries paused, skipping${swarmNote}`);
    notify(`⏸ ${isPaper ? '[PAPER] ' : ''}*${best.symbol ?? best.mint.slice(0, 8)}* skipped — market in dump regime (entries paused)`);
    return;
  }
  if (regimeMultiplier !== 1.0) {
    const oldBudget = finalBudget;
    finalBudget *= regimeMultiplier;
    log('info', `Regime sizing: ${best.symbol ?? best.mint.slice(0, 8)} → ×${regimeMultiplier} (${oldBudget.toFixed(4)} → ${finalBudget.toFixed(4)} SOL)${swarmNote}`);
  }

  // Defensive floor: never reach the swap with a non-positive budget, whatever combination of
  // herd penalty, consensus boost and regime multiplier produced it (also guards against any
  // future regression re-introducing a phantom 0-cost position).
  if (!(finalBudget > 0)) {
    log('warn', `Non-positive entry budget for ${best.symbol ?? best.mint.slice(0, 8)} — skipping entry`, { finalBudget });
    return;
  }
  ...
  try {
    ...
    // Momentum-agent (agent2) signal capture: snapshot smart-money / rug / holder signals at entry so
    // we can later backtest whether they separate winners — LOGGED, not (yet) gated. Best-effort:
    // NEVER block or fail an entry on capture (we've already filled). Uses paid x402 calls, so it needs
    // agent2 to have CIRC (or an internal-key path); on failure the fields are null and nothing is lost.
    let x402Signals = null;
    if (cfg.momentum?.captureSignals !== false && cfg.strategy?.scorer === 'momentum') {
      try {
        const [tt, sec, hold] = await Promise.all([
          api.tokenTopTraders(best.mint).catch(() => null),
          api.tokenSecurity(best.mint).catch(() => null),
          api.tokenHolders(best.mint).catch(() => null),
        ]);
        const trim = (a, n) => Array.isArray(a) ? a.slice(0, n) : null;
        x402Signals = {
          topTraders: tt   && !tt.error   ? { count: tt.count ?? tt.traders?.length ?? null, sample: trim(tt.traders, 10) } : null,
          security:   sec  && !sec.error  ? sec : null,
          holders:    hold && !hold.error ? { count: hold.count ?? hold.holderCount ?? hold.total ?? null, top: trim(hold.holders ?? hold.top, 5) } : null,
          capturedAt: Date.now(),
        };
      } catch { /* best-effort — capture must never disrupt trading */ }
    }
    ...
    const opened = positions.openPosition(best.mint, {
      ...
      entryConditions: {
        ...
        scanSource,
        x402:            x402Signals,   // momentum-agent: smart-money / rug / holder snapshot at entry
      },
    });
    if (!opened) {
...
module.exports = { start, pickScorer };
