...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  // Geyser dexLosers — sub-second gRPC stream of mints currently dipping. Fetched FIRST so
  // it both (a) seeds the scan universe (the strategy hunts dips, but the volume-ranked
  // trending list often omits them — this makes the live feed an actual dip trigger), and
  // (b) gives a +5 score boost to candidates confirmed dipping by two independent sources.
  ...
  try {
    let result = await api.scanFree({ limit: 30, minLiquidity, seedMints: [..._geyserDipMints] });
  ...
  try {
    ...
    const opened = positions.openPosition(best.mint, {
      ...
      entryConditions: {
        ...
        scanSource,
        geyserConfirmed: best._geyser ?? false,
      },
    });
    if (!opened) {
