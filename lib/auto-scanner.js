...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  // Buy-pressure gate — cap chasing: entries with >65% buy ratio in 5m show 26% WR vs 55-65%
  // band at 65% WR. Prevents scoring from over-rewarding ticker chasing on thin volume.
  // Tunable: strategy.maxBuyRatioPct (default 65, null = disabled).
  const maxBuyRatioPct = s.maxBuyRatioPct ?? 65;
  if (maxBuyRatioPct != null) {
    const buyRatio5m = best.txns5m && (best.txns5m.buys ?? 0) + (best.txns5m.sells ?? 0) > 0
      ? best.txns5m.buys / ((best.txns5m.buys ?? 0) + (best.txns5m.sells ?? 0)) * 100
      : null;
    if (buyRatio5m != null && buyRatio5m > maxBuyRatioPct) {
      log('info', `Buy-pressure gate: ${best.symbol ?? best.mint.slice(0, 8)} buyRatio5m ${buyRatio5m.toFixed(0)}% > ${maxBuyRatioPct}% (chasing detected) — skipping`);
      notify(`⏭ *${best.symbol ?? best.mint.slice(0, 8)}* skipped — buy-pressure too high (${buyRatio5m.toFixed(0)}%, threshold ${maxBuyRatioPct}%)`);
      return;
    }
  }
