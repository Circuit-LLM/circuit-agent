...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  if (swarmBlacklisted.size > 0) {
    ...
    for (const c of scored) {
      ...
      try {
        const info = await api.tokenInfo(c.mint);
        const verdict = (info.verdict ?? info.rugRisk ?? 'unknown').toUpperCase();
        if (verdict === 'DANGER') {
  ...
  // The boost only applies when our own independent scorer also rates the candidate
  // highly — this prevents a swarm hallucination from amplifying a borderline trade.
  ...
  try {
    ...
    if (consensus?.consensus === 'bullish' && consensus.agents >= 2) {
      ...
      try {
        const rugInfo = await api.tokenInfo(best.mint);
        const rugVerdict = (rugInfo.verdict ?? rugInfo.rugRisk ?? 'unknown').toUpperCase();
