...
async function runCycle(api, wallet, swap, cfg, notify) {
  ...
  try {
    ...
    rugVerdict = _tokenInfo.risk?.verdict ?? _tokenInfo.verdict ?? _tokenInfo.rugRisk ?? rugVerdict;
  ...
  try {
    ...
    // outAmount is raw atomic units — divide by 10^decimals to get UI amount so
...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  const scheduleNext = () => {
    ...
    const nextIntervalMs = loadConfig().strategy?.scanIntervalMs ?? intervalMs;
