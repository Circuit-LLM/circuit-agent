...
class PaperSwapExecutor {
  ...
  async getTokenBalance(mint) {
    ...
    // ok:true always — paper mode reads local state, so the balance is never "unknown"
    // (mirrors the real SwapExecutor contract; monitor.js checks ok === false).
    if (!pos) return { rawAmount: BigInt(0), amount: '0', uiAmount: 0, decimals: 6, ok: true };
