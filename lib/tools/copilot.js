...
// Friendly names for DexScreener/RugCheck market types.
const DEX_NAMES = {
  pump_fun_amm: 'Pump.fun AMM', pumpswap: 'PumpSwap', meteora_damm_v2: 'Meteora DAMM v2',
  meteoradlmm: 'Meteora DLMM', meteora: 'Meteora', raydium_clmm: 'Raydium CLMM',
  raydium_cpmm: 'Raydium CPMM', raydium: 'Raydium', orca: 'Orca', whirlpool: 'Orca Whirlpool',
};
const _dexName = (t) => DEX_NAMES[String(t || '').toLowerCase()] ?? (t ? String(t).replace(/_/g, ' ') : 'Pool');

const TOKEN_PROGRAM      = 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA';
const TOKEN_2022_PROGRAM = 'TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb';

// Accurate holder count = unique wallets with a non-zero balance, read on-chain.
// RugCheck's totalHolders counts all-time accounts (644 for a token every other
// source shows as ~282) — this matches Solscan/Birdeye. Best-effort: one
// getProgramAccounts call, ~750ms for a few-hundred-holder token; falls back to
// null (→ RugCheck / Solscan link) on timeout or for very large tokens.
async function _countHolders(connection, mint, isToken2022) {
  if (!connection) return null;
  try {
    const { PublicKey } = require('@solana/web3.js');
    const programId = new PublicKey(isToken2022 ? TOKEN_2022_PROGRAM : TOKEN_PROGRAM);
    const accts = await Promise.race([
      ...
      new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 6_000)),
    ]);
    if (accts.length > 60_000) return null;   // backstop: too many to process in-request
    const owners = new Set();
    for (const a of accts) {
      const b = a.account.data;                  // 40-byte Buffer
      if (b.readBigUInt64LE(32) > 0n) owners.add(b.subarray(0, 32).toString('hex'));
    }
    return owners.size;
  } catch { return null; }
}
...
async function _labelProgramHolders(connection, top) {
  ...
  try {
    ...
    const infos = await Promise.race([
      connection.getMultipleAccountsInfo(targets.map(h => new PublicKey(h.owner))),
...
async function buildDossier(api, mint, connection = null) {
  ...
  out.token = {
    ...
    logo:        dx?.imageUrl ?? ov?.meta?.logoURI ?? null,
    ageHours:    null,
  };
  ...
  // ── Liquidity (per-market breakdown; tokens can have hundreds of pools) ─────
  if (rp && Array.isArray(rp.markets)) {
    const markets = rp.markets.map(m => ({
      ...
      lpLockedPct:  m.lpLockedPct ?? m.lp?.lpLockedPct ?? null,
    ...
    out.liquidity = {
      ...
      marketCount: markets.length,          // full count so the UI can show "+N more"
      markets:     markets.slice(0, 6),     // top 6 by liquidity
    };
  } else if (ov?.liquidity != null) {
    out.liquidity = { totalUsd: ov.liquidity, markets: [], marketCount: 0, lpProviders: null };
  }
  ...
  if (rp && Array.isArray(rp.topHolders) && rp.topHolders.length) {
    ...
    // Pool vault authorities: RugCheck's knownAccounts labels some AMM accounts but
    // misses others (e.g. a Meteora DAMM v2 vault authority shows as a plain
    // System-owned wallet). The markets data is authoritative — each pool's
    // liquidity-vault `owner` is the pool authority — so map those to the DEX.
    const poolAuth = {};
    for (const mk of (rp.markets ?? [])) {
      for (const acct of [mk.liquidityAAccount, mk.liquidityBAccount]) {
        if (acct?.owner) poolAuth[acct.owner] = mk.marketType ?? 'pool';
      }
    }
    const labeled = rp.topHolders.map((h, i) => {
      const k = known[h.owner];
      let infra = k?.name ?? null, infraType = k?.type ?? null;
      if (!infra && poolAuth[h.owner]) { infra = _dexName(poolAuth[h.owner]) + ' Pool'; infraType = 'AMM'; }
      return {
        rank:      i + 1,
        owner:     h.owner,
        pct:       h.pct ?? 0,
        ...
        insider:   !!h.insider,
        infra,
        infraType,   // AMM | LOCKER | CREATOR | (later PROGRAM/PROTOCOL)
      };
    });
    const top = labeled.slice(0, 10);
    const isToken2022 = ov?.security?.isToken2022
      ?? (typeof rp?.tokenProgram === 'string' && rp.tokenProgram.startsWith('Tokenz'));
    // Only scan on-chain for the exact holder count when the token is small enough
    // that the RPC response can't stall the event loop — a mega-token's response is
    // tens of MB and web3.js JSON-parses it synchronously, freezing the agent. Use
    // RugCheck's rough count to gate; large/unknown tokens keep RugCheck's number.
    const rugCount  = typeof rp.totalHolders === 'number' ? rp.totalHolders : null;
    const scanSafe  = rugCount != null && rugCount > 0 && rugCount <= 20_000;
    // Run both on-chain enrichments in parallel; each is independently bounded.
    const [onchainCount] = await Promise.all([
      scanSafe ? _countHolders(connection, mint, isToken2022) : Promise.resolve(null),
      _labelProgramHolders(connection, top),
    ]);

    const real = top.filter(h => !h.infra);   // exclude pools, lockers, programs
    out.holders = {
      total:        onchainCount ?? (rp.totalHolders || null),
      totalSource:  onchainCount != null ? 'on-chain' : 'RugCheck',
      totalApprox:  onchainCount == null,      // RugCheck count is not exact
  ...
  out.security = {
    ...
    // RugCheck's on-chain tokenMeta.mutable is authoritative; circuit's `immutable`
    // is unreliable (reports true even for tokens with an active update authority).
    immutable:       rp?.tokenMeta ? (rp.tokenMeta.mutable === false) : (ov?.security?.immutable ?? null),
    transferFeePct:  rp?.transferFee?.pct ?? null,
    flags,
  };

  // ── Market activity (momentum + flow), from DexScreener ────────────────────
  if (dx) {
    const t24 = dx.txns?.h24 ?? {};
    out.market = {
      change1h:  dx.priceChange?.h1  ?? null,
      change6h:  dx.priceChange?.h6  ?? null,
      change24h: dx.priceChange?.h24 ?? null,
      volume24h: dx.volume?.h24 ?? null,
      buys24h:   t24.buys ?? null,
      sells24h:  t24.sells ?? null,
      fdv:       dx.fdv ?? null,
      pairUrl:   dx.url ?? null,
    };
    // Prefer DexScreener FDV for market cap when the feed-derived one is absent.
    if (out.price && out.price.marketCap == null && dx.marketCap != null) out.price.marketCap = dx.marketCap;
  }

  // ── Socials / links (DexScreener is the reliable source) ───────────────────
  const socials = {};
  for (const w of (dx?.websites ?? [])) { if (!socials.website) socials.website = w.url; }
  for (const s of (dx?.socials ?? [])) {
    const ty = String(s.type || '').toLowerCase();
    if (ty === 'twitter' && !socials.twitter)   socials.twitter = s.url;
    else if (ty === 'telegram' && !socials.telegram) socials.telegram = s.url;
    else if (ty === 'discord' && !socials.discord)   socials.discord = s.url;
  }
  if (!socials.website && ov?.meta?.website) socials.website = ov.meta.website;
  if (!socials.twitter && ov?.meta?.twitter) socials.twitter = ov.meta.twitter;
  const desc = ov?.meta?.description ?? null;
  if (desc) socials.description = desc;
