...
async function buildDossier(api, mint) {
  ...
  if (rp && Array.isArray(rp.topHolders) && rp.topHolders.length) {
    ...
    const labeled = rp.topHolders.map(h => {
      const k = known[h.owner];
      return {
        owner: h.owner, pct: h.pct ?? 0, insider: !!h.insider,
        infra: k?.name ?? null, infraType: k?.type ?? null,
      };
    });
    const real = labeled.filter(h => !h.infra);   // exclude AMM pools, lockers, etc.
    out.holders = {
