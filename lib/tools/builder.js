...
const BLOCKED_BASH = [
  ...
  /rm\s+-[rRfF]+\s*~/,                 // rm -rf ~ (home directory wipe)
  />\s*\/etc\//,                        // overwrite /etc/ files
  /chmod\s+777\s+\/(?!\s)/,             // chmod 777 /
  /curl.*\|\s*(bash|sh|zsh)/,           // pipe-to-shell from internet
  // Download to file then execute — bypasses the pipe-to-shell pattern above
  /curl[^;\n]+-o\s+\S[^;\n]*(?:&&|;)[^;\n]*\b(?:bash|sh|zsh|node|python3?)\b/,
  ...
  /\b(?:cat|less|more|head|tail|bat|nano|vi|vim|awk)\b[^|]*\.env/,
  /\bgrep\b[^|]*\.env/,               // grep for keys inside .env*
  // Script-interpreter reads of .env — not covered by the cat/vim pattern above.
  // python3 -c "open('.env').read()", node -e "fs.readFileSync('.env')", etc.
  /\b(?:python3?|node|ruby|perl|php)\b[^|;\n]*\.env/,
  // curl --data @file or -d @file — exfiltrates file contents to a remote server
  /\bcurl\b[^;\n]*\s-d\s+@/,
  /\bcurl\b[^;\n]*--data[- ][^;\n]*@/,
  // Crontab backdoor — (crontab -l; echo "...") | crontab or crontab -e
  /\|\s*crontab\b/,
  /\bcrontab\s+-e\b/,
];
...
const HANDLERS = {
  async read_file(args, _ctx, _log) {
    ...
    // Resolve symlinks before applying the sensitive-dir denylist.
    // Without this, ~/mylink → ~/.ssh bypasses the prefix check entirely.
    // For non-existent paths, resolve the parent (the file may not exist yet;
    // existsSync below will handle that gracefully).
    ...
    try {
      ...
      } catch { /* parent also missing — leave realTarget as target */ }
    }

    // After symlink resolution, re-enforce the HOME_ROOT boundary.
    // A symlink could point anywhere on the filesystem; reject escapes.
    if (!realTarget.startsWith(HOME_ROOT + path.sep) && realTarget !== HOME_ROOT) {
      return JSON.stringify({ error: 'Cannot read path — resolves outside home directory.' });
    }

    // Block sensitive home-directory trees (SSH keys, cloud credentials, GPG keys, etc.)
    // Checked against the resolved path so symlinks cannot bypass the denylist.
    const relToHome = path.relative(HOME_ROOT, realTarget);
    if (relToHome.startsWith('..') ||
        BLOCKED_READ_DIRS.some(d => relToHome === d || relToHome.startsWith(d + path.sep))) {
  ...
  async write_file(args, _ctx, log) {
    ...
    // Mirror the read_file sensitive-directory protection for writes.
    // write_file accepts absolute paths under HOME_ROOT (e.g. ~/circuit-node/routes/foo.js),
    // so without this check an LLM could overwrite ~/.ssh/authorized_keys to add a backdoor.
    const relToHome = path.relative(HOME_ROOT, target);
    if (BLOCKED_READ_DIRS.some(d => relToHome === d || relToHome.startsWith(d + path.sep))) {
      return JSON.stringify({ error: 'Cannot write to security-sensitive directory.' });
    }
    // Block common shell/tool config files directly in HOME_ROOT — writing these can
    // persist malicious commands across agent restarts or inject env modifications.
    const BLOCKED_HOME_FILES = [
      '.bashrc', '.bash_profile', '.bash_logout', '.profile',
      '.zshrc', '.zshenv', '.zprofile',
      '.gitconfig', '.npmrc', '.netrc', '.pypirc',
    ];
    if (path.dirname(target) === HOME_ROOT && BLOCKED_HOME_FILES.includes(path.basename(target))) {
      return JSON.stringify({ error: `Cannot overwrite shell/tool configuration file: ${path.basename(target)}` });
    }
  ...
  async install_package(args, _ctx, log) {
    ...
    // --ignore-scripts prevents npm lifecycle scripts (preinstall/postinstall) from
    // executing arbitrary code at install time. If a package legitimately needs its
    // build step, the agent can run it manually with run_script after reviewing the code.
    const result = spawnSync('npm', ['install', '--ignore-scripts', pkg], {
      cwd:       AGENT_ROOT,
      timeout:   120_000,
      maxBuffer: 2 * 1024 * 1024,
      encoding:  'utf8',
