...
// The skill list in this tool's description is what the trading brain actually
// sees, so it is built from disk at load time rather than hand-maintained —
// a stale list here silently hides skills (risk-management among them) from the
// model unless it happens to call list_skills first.
function _skillNamesOrFallback() {
  const names = skills.skillNames();
  return names.length ? names : ['dip-reversal'];   // empty only if skills/ is unreadable
}

function _skillListText() {
  const names = skills.skillNames();
  return names.length
...
const DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      name: 'load_skill',
      description: `Load a specialized skill/strategy context to guide your current task. ${_skillListText()} Load "nft" for anything about Solana NFT collections, floors, or NFT arbitrage. Call list_skills for one-line descriptions of each.`,
      parameters: {
        type: 'object',
        properties: {
          skill: { type: 'string', description: `Skill name — one of: ${_skillNamesOrFallback().join(', ')}` },
        },
        required: ['skill'],
      },
    },
...
const HANDLERS = {
  ...
  async load_skill(args, ctx, log) {
    const { skill } = args;
    const loaded = skills.readSkill(skill);

    if (!loaded) {
      // Same response for a malformed name and a missing one — the model only
      // needs to know which names are valid.
      return JSON.stringify({
        error: `Skill '${skill}' not found. Available: ${skills.skillNames().join(', ')}`,
      });
    }

    log('info', `Skill loaded: ${loaded.name}`, loaded.truncated ? { truncated: true, bytes: loaded.bytes } : undefined);

    // Usage log — the input side of the skill-performance pipeline. Records the
    // cycle this load belongs to so grading can attribute entries that follow it
    // rather than blaming every trade that happened to close the same day.
    try {
      const tracker = require('../memory/skill-tracker');
      tracker.logSkillUsage(loaded.name, 'loaded', {
        decisionId: ctx?.decisionId ?? null,
        source:     ctx?.source ?? null,
      });
    } catch (err) {
      log('warn', `Skill usage not logged: ${err.message}`);
    }

    // Record on the agent profile that this skill has been used at least once.
    try {
      const { loadProfile, saveProfile } = require('../profile');
      const profile = loadProfile();
      if (!profile) {
        log('warn', 'Profile missing — skill not added to specialization (run: node agent.js init)');
      } else if (!profile.specialization?.skills?.includes(loaded.name)) {
        profile.specialization = profile.specialization ?? {};
        profile.specialization.skills = profile.specialization.skills ?? [];
        profile.specialization.skills.push(loaded.name);
        saveProfile(profile);
        log('info', `Profile updated: added skill '${loaded.name}'`);
      }
    } catch (err) {
      log('warn', `Profile not updated for skill '${loaded.name}': ${err.message}`);
    }
