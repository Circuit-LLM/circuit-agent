...
const HANDLERS = {
  ...
  async update_config(args, ctx, log) {
    ...
    let upsertIdx;
    const _mcfg = require('../config').loadConfig();
    if (_mcfg.memory?.enabled && _mcfg.memory?.proceduralHistory) {
      // Keep the full reasoning trail instead of overwriting by param.
      require('../memory/procedural').appendWithHistory(suggestions, entry);
      upsertIdx = suggestions.length - 1; // append keeps the new entry last
    } else {
      // OFF: today's exact behavior — upsert by param, no history kept.
      ...
      upsertIdx = existing >= 0 ? existing : suggestions.length - 1;
    }
  ...
  async set_session_strategy(args, ctx, log) {
    const { saveStrategy, loadStrategy } = require('../agent-loop');
    ...
    // Grade the outgoing strategy before overwriting it, and stamp a fresh setAt — mirror the
    // agent-loop path so a manual (chat/Telegram) strategy change opens its OWN plan-grading
    // window. Without this, saveStrategy's `...current` spread made the manual write INHERIT the
    // previous strategy's setAt, which merged windows and skipped grading the outgoing strategy.
    // Gated + best-effort exactly like agent-loop — never block the strategy change.
    if (ctx?.cfg?.memory?.enabled && ctx?.cfg?.memory?.planGrading) {
