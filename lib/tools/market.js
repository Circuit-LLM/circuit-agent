...
const HANDLERS = {
  ...
  async token_holders(args, ctx, _log) {
    ...
    if (!data || data.error) return JSON.stringify(data ?? { error: 'Holder data unavailable' });

    // Known Solana program IDs for infrastructure (scalable by program, not per-token)
    const KNOWN_PROGRAMS = {
      'PumpFun2yQ9agjqVVRnKYvjxZaerlmVGRxVBmShkuX': '[PUMP.FUN LP]',
      'MeteoraDLMm846bnHiQTCzvNivJJtYzcxM7zZ5B1B6': '[METEORA LP]',
      'MarinadeFinance11111111111111111111111111': '[MARINADE STAKING]',
      'JitoSOL1111111111111111111111111111111111111': '[JITO STAKING]',
      'Stake11111111111111111111111111111111111111': '[NATIVE STAKING]',
      'TokenzQdBbjFD8VPL1Nrng3XGL7G5PzwAjYr7RTm5P': '[TOKEN PROGRAM]',
      'TokenkegQfeZyiNwAJsyFbPVwwQnoxwUV9zWZwm3J6': '[TOKEN-2022 PROGRAM]',
    };

    // Check if API returned ownership info; if not, label is based on heuristics
    if (data.holders && Array.isArray(data.holders)) {
      data.holders = data.holders.map(h => {
        // If account has ownership info, use it to detect program type
        const ownerProgram = h.owner;
        const label = ownerProgram ? KNOWN_PROGRAMS[ownerProgram] : null;

        if (label) {
          return { ...h, address: `${label} ${h.address}` };
        }
        return h;
      });
    }

    // Add context note: explain that high concentration may include infrastructure
    const mayHaveInfra = (data.holders || []).some(h => h.owner && KNOWN_PROGRAMS[h.owner]);
    if (mayHaveInfra) {
      data._note = 'Infrastructure contracts (LPs, staking programs) are labeled with [BRACKETS]. Real holder concentration (excluding infrastructure) may be significantly lower.';
    }
