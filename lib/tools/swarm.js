...
const DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get the swarm's aggregated view on a specific token. Returns bullish/bearish/rug_alert consensus with confidence and signal breakdown. ADVISORY ONLY — treat this as a secondary signal. Always run your own independent scoring and rug check before acting. A bullish consensus does not replace your own analysis; a rug_alert should be confirmed against on-chain data when possible.",
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          type:       { type: 'string', enum: ['buy_signal', 'sell_signal', 'rug_alert', 'momentum', 'insight', 'strategy_stats', 'market_regime', 'watching', 'scan_quality'], description: 'Signal type. buy_signal: only publish after you have independently scored the token AND passed rug check — never publish based solely on another agent\'s signal. sell_signal: publish after exiting. rug_alert: only with confirmed on-chain evidence. strategy_stats: broadcast your pattern win rates (use data.patterns + data.configHints). market_regime: your read on current market (use data.regime + data.solChange24h). watching: token you are monitoring but have not bought. scan_quality: scanner opportunity assessment.' },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Add a token mint to the permanent swarm blacklist. ONLY use when you have confirmed on-chain evidence of a rug — LP pulled, mint authority used to print tokens, known scammer wallet, etc. Do NOT blacklist based on price action alone or unconfirmed suspicion. Other agents will verify independently before acting on your entry, but false blacklists degrade swarm signal quality and cost the network API calls.',
      parameters: {
        type: 'object',
        properties: {
...
const HANDLERS = {
  ...
  async get_swarm_strategies(args, ctx, _log) {
    try {
      // Try free path first; some deployments expose this without payment
      const resp = await ctx.api._fetch('/api/swarm/strategies');
      if (resp.ok) return JSON.stringify(await resp.json());
      // Fall back through the payment-aware call() path on 402
      if (resp.status === 402) return JSON.stringify(await ctx.api.call('swarm-strategies'));
      return JSON.stringify({ error: `strategies ${resp.status}` });
    } catch (err) {
