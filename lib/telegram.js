...
function start(token, agentCtx) {
  ...
  bot.command('start', ctx => {
    ...
    if (!_ownerChatId) {
      ...
      return ctx.reply(
        ...
        '/research \\<mint\\> — token dossier \\(price, rug, swarm view\\)\n' +
        '/watches — list active alerts \\+ follows\n' +
        '/follow \\<wallet\\> — copy\\-signal a wallet on new entries\n' +
    ...
    if (chatId !== _ownerChatId) {
      ...
      return ctx.reply('This agent already has an owner.');
    }
  ...
  bot.command('help', ctx => {
    ...
    // NOTE: MarkdownV2 reserves _ * [ ] ( ) ~ ` > # + - = | { } . ! — every literal
    // one must be backslash-escaped or Telegram 400s. Return the promise so a
    // formatting slip routes to bot.catch instead of crashing the process.
    return ctx.reply(
      '*circuit\\-agent commands*\n\n' +
      '/wallet — check SOL \\+ CIRCUIT balances\n' +
      '/status — show open positions \\+ P\\&L\n' +
      '/scan — run a market scan\n' +
      '/research \\<mint\\> — token dossier\n' +
      '/watches — list alerts \\+ follows\n' +
      '/follow \\<wallet\\> — copy a wallet\n' +
  ...
  bot.command('research', async ctx => {
    ...
    if (!arg) { ctx.reply('Usage: /research <mint or token question>'); return; }
    const sender   = ctx.from?.first_name ?? ctx.from?.username ?? String(ctx.from?.id);
    const senderId = String(ctx.from?.id);
    enqueue('telegram', sender, senderId,
      `Research this token and give me a compact dossier card (price, security verdict, ` +
      `liquidity, swarm view, blacklist status, your take): ${arg}. ` +
      `Use the research_token tool. If I only gave a symbol, find the mint first.`,
      `research_${Date.now()}`);
  ...
  bot.command('follow', ctx => {
    ...
    if (!addr) { ctx.reply('Usage: /follow <wallet address> — alerts you when it enters a new token'); return; }
    try {
      const w = require('./watches').addFollowWatch({ address: addr });
      ctx.reply(`Following ${addr.slice(0, 4)}…${addr.slice(-4)} — I'll alert you on new token entries. (${w.id})`);
    } catch (e) { ctx.reply('Could not add follow: ' + e.message); }
  });

  bot.command('watches', ctx => {
    if (!isOwner(ctx)) return;
    try {
      const all = require('./watches').list();
      if (!all.length) { ctx.reply('No active watches. Ask me to "alert when <token> hits $X" or "watch wallet <address>".'); return; }
      const lines = all.map(w => w.type === 'price'
        ? `• ${w.symbol ?? w.mint.slice(0, 6) + '…'} ${w.aboveUsd != null ? '≥ $' + w.aboveUsd : ''}${w.belowUsd != null ? '≤ $' + w.belowUsd : ''} (${w.id})`
        : `• wallet ${w.address.slice(0, 4)}…${w.address.slice(-4)}${w.note ? ' — ' + w.note : ''} (${w.id})`);
      ctx.reply('Active watches:\n' + lines.join('\n'));
    } catch (e) { ctx.reply('Could not read watches: ' + e.message); }
  });
...
// Owner chat ID claimed via /start — used as the notification fallback when
// telegram.heartbeatChatId is not configured (previously: silent zero notifications).
function getOwnerChatId() { return _ownerChatId ?? null; }

module.exports = { start, getOwnerChatId };
