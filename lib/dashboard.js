  ...
  try {
    ...
    if (route === '/api/watches/add-price' && req.method === 'POST') {
      ...
      const timeout = setTimeout(() => {
        console.error('[WATCH-API] TIMEOUT: add-price req.on(end) never fired');
        if (!res.headersSent) {
          res.writeHead(500);
      ...
      req.on('error', (err) => {
        clearTimeout(timeout);
        console.error('[WATCH-API] add-price error', { error: err.message });
        if (!res.headersSent) {
          res.writeHead(500);
          res.end(JSON.stringify({ error: err.message }));
        }
    ...
    if (route === '/api/watches/add-wallet' && req.method === 'POST') {
      console.log('[WATCH-API] add-wallet endpoint called');
      let body = '';
      const timeout = setTimeout(() => {
        console.error('[WATCH-API] TIMEOUT: req.on(end) never fired after 5s');
        if (!res.headersSent) {
          res.writeHead(500);
          res.end(JSON.stringify({ error: 'Request timeout - stream end never fired' }));
        }
      }, 5000);

      req.on('data', chunk => {
        console.log('[WATCH-API] received data chunk', { bytes: chunk.length });
        body += chunk;
      });

      req.on('end', () => {
        clearTimeout(timeout);
        console.log('[WATCH-API] request end, parsing body', { bodyLength: body.length });
        try {
          const parsed = JSON.parse(body);
          console.log('[WATCH-API] parsed body', { address: parsed.address });
          const w = watches.addWalletWatch(parsed);
          console.log('[WATCH-API] watch added successfully', { id: w.id });
          res.end(JSON.stringify({ ok: true, watch: w }));
        } catch (err) {
          console.error('[WATCH-API] error', { error: err.message });
          res.writeHead(400);
          res.end(JSON.stringify({ error: err.message }));
        }
      });
      ...
      req.on('error', (err) => {
        clearTimeout(timeout);
        console.error('[WATCH-API] request error', { error: err.message });
        if (!res.headersSent) {
          res.writeHead(500);
          res.end(JSON.stringify({ error: err.message }));
        }
