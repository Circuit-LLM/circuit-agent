...
async function _handleRequest(req, res) {
  ...
  // Request body is read ONCE here for every method that carries one (POST/PATCH/PUT)
  // and parsed into `body`. Route handlers below must use this `body` — they must NOT
  // re-attach req.on('data'/'end'), because the stream is already fully consumed by the
  // time they run, so a second 'end' listener never fires (the request would hang).
  // 64 KB cap prevents memory exhaustion from large payloads.
  const methodHasBody = req.method === 'POST' || req.method === 'PATCH' || req.method === 'PUT';
  const body = await new Promise(resolve => {
    if (!methodHasBody) return resolve({});
    let buf = '';
    req.on('data', c => {
      buf += c;
  ...
  try {
    if (route === '/api/status') {
      ...
      // Regime state (Tier 7, live from lib/analysis/regime-state.js)
      let regime = null;
      try {
        const regimeState = require('./analysis/regime-state');
        const state = regimeState.loadRegimeState();
        regime = state ? { regime: state.regime, confidence: state.confidence, reasoning: state.reasoning } : null;
      } catch {}
    ...
    if (route === '/api/config') {
      ...
      if (req.method === 'GET') {
        ...
        const groups = [
          {id:'trading-setup', label:'Trading Setup', purpose:'Define what tokens to buy and minimum quality thresholds', settings:['strategy'], subGroups:[]},
          {id:'exit-mgmt', label:'Exit & Position Management', purpose:'Configure stop-loss, take-profit, and position exit rules', settings:['strategy'], subGroups:[]},
          {id:'risk-filters', label:'Risk Filters & Token Safety', purpose:'Avoid risky tokens with automated safety checks', settings:['strategy','copilot'], subGroups:[]},
          {id:'portfolio', label:'Portfolio Management', purpose:'Rebalance concentrations and manage multi-position strategies', settings:['strategy'], subGroups:[], new:true},
          {id:'yields', label:'Yield Discovery', purpose:'Find and auto-deploy profitable yield opportunities', settings:['strategy'], subGroups:[], new:true},
          {id:'alerts', label:'Alerts & Monitoring', purpose:'Watch prices, wallets, and token research', settings:['copilot'], subGroups:[]},
          {id:'execution', label:'Execution & Fees', purpose:'Configure swap execution, Jito bundles, and priority fees', settings:['strategy'], subGroups:[]},
          {id:'wallet-safety', label:'Wallet Safety & Reserves', purpose:'Protect wallet with gas reserves and loss limits', settings:['strategy'], subGroups:[]},
          {id:'agent-loop', label:'Strategy Reasoning & Mode', purpose:'Configure LLM cycle timing and reasoning parameters', settings:['agentLoop','llm'], subGroups:[]},
          {id:'swarm', label:'Multi-Agent Swarm', purpose:'Coordinate with other agents and share signals', settings:['swarm'], subGroups:[]},
          {id:'learning', label:'Self-Improvement & Learning', purpose:'Enable reflection cycles and memory-based optimization', settings:['reflect'], subGroups:[]},
          {id:'heartbeat', label:'Heartbeat & Notifications', purpose:'Configure portfolio updates and Telegram alerts', settings:['heartbeat'], subGroups:[]},
          {id:'system', label:'System & Advanced', purpose:'Dashboard, API, and infrastructure settings', settings:['dashboard','circuit','telegram'], subGroups:[]},
        ];
    ...
    if (route === '/api/watches/remove' && req.method === 'POST') {
      try {
        const removed = watches.removeWatch(String(body.id ?? ''));
    ...
    if (route === '/api/copilot/research' && req.method === 'POST') {
      try {
        ...
        // Base58 excludes 0/O/I/l but INCLUDES lowercase a-k, m-z — the prior
        // uppercase-only pattern rejected every real mint (all contain lowercase).
        ...
        const dossier = await buildDossier(_api, mint, _wallet?.connection ?? null);
    ...
    if (route === '/api/watches/add-price' && req.method === 'POST') {
      try {
        const w = watches.addPriceWatch(body);
    ...
    if (route === '/api/watches/add-wallet' && req.method === 'POST') {
      try {
        const w = watches.addWalletWatch(body);
        res.end(JSON.stringify({ ok: true, watch: w }));
      } catch (err) {
        res.writeHead(400);
        res.end(JSON.stringify({ error: err.message }));
    ...
    if (route === '/api/watches/add-follow' && req.method === 'POST') {
      try {
        const w = watches.addFollowWatch(body);
        res.end(JSON.stringify({ ok: true, watch: w }));
      } catch (err) {
        res.writeHead(400);
        res.end(JSON.stringify({ error: err.message }));
