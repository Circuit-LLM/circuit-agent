...
async function _fetchDexPrices(mints) {
  ...
  // HTTP /api/positions hit DexScreener within the same 12s window.
  ...
  try {
    ...
    for (const mint of mints) {
      const tp = pairs.filter(p => p.baseToken?.address === mint);
      if (!tp.length) continue;
      const best = tp.sort((a, b) => (b.liquidity?.usd ?? 0) - (a.liquidity?.usd ?? 0))[0];
      out[mint] = {
...
async function _handleRequest(req, res) {
  ...
  if (apiKey) {
    ...
    if (provided !== apiKey && route !== '/' && route !== '/dashboard') {
  ...
  try {
    ...
    if (route === '/api/config') {
      ...
      res.end(JSON.stringify({ config: live }));
      return;
    }
