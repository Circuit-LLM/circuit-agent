...
class CircuitClient {
  ...
  async scanFree(opts = {}) {
    ...
    try {
      ...
      const trendMints = (trendData.tokens ?? []).map(t => t.mint).filter(m => m && m !== WSOL);
      // seedMints (currently-dipping tokens from the Geyser dexLosers feed) are placed FIRST
      // so they are always evaluated — the strategy hunts dips, but the volume-ranked trending
      // list often omits them. De-dupe while preserving the seed-first ordering.
      const topMints  = [...new Set([...seedMints.filter(m => m && m !== WSOL), ...trendMints])];
