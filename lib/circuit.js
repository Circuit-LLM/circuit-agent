...
class CircuitClient {
   ...
   */
  async scanFree(opts = {}) {
    const { limit = 20, minLiquidity = 5000 } = opts;
    const feedBase = 'http://127.0.0.1:18941'; // circuit-price-feed, same VPS
    try {
      // Step 1: top mints by on-chain SOL volume (Raydium CPMM swaps via Geyser)
      const trendResp = await fetch(`${feedBase}/trending?limit=50`, { signal: AbortSignal.timeout(5_000) });
      if (!trendResp.ok) throw new Error(`price-feed /trending ${trendResp.status}`);
      const trendData = await trendResp.json();
      const solUsd    = trendData.solUsd ?? 150;
      const topMints  = (trendData.tokens ?? []).map(t => t.mint).filter(Boolean);
      ...
      // Step 3: per-mint OHLCV candles for price change + txn data
      const candidates = (await Promise.all(topMints.slice(0, limit * 2).map(async (mint) => {
        try {
          const [r5m, r1h] = await Promise.all([
            ...
            fetch(`${feedBase}/candles/${mint}?window=1h&limit=2`,  { signal: AbortSignal.timeout(3_000) }),
          ]);
          ...
          const last5m        = c5[c5.length - 1];
          const priceChange5m = c5.length >= 2 ? ((last5m.c - c5[0].o) / c5[0].o) * 100 : 0;
          const priceChange1h = c1.length >= 2 ? ((c1[c1.length - 1].c - c1[0].o) / c1[0].o) * 100 : 0;
          const buys5m        = c5.reduce((s, c) => s + (c.b ?? 0), 0);
          const sells5m       = c5.reduce((s, c) => s + (c.s ?? 0), 0);
          const vol5m         = c5.reduce((s, c) => s + (c.v ?? 0), 0);
  ...
  async tokenInfoFree(mint) {
    try {
      ...
      return {
        ...
        verdict: d.score <= 300 ? 'DANGER' : d.score <= 700 ? 'CAUTION' : 'OK',
        rugRisk: d.score <= 300 ? 'DANGER' : d.score <= 700 ? 'CAUTION' : 'OK',
        rugScore: d.score ?? null,
        risks:   (d.risks ?? []).map(r => r.name),
