    ...
    try {
      ...
      const candidates = (await Promise.all(topMints.slice(0, scanBudget).map(async (mint) => {
        try {
          ...
          // ── Data-quality metadata (feed honesty) ────────────────────────────────
          // priceChange5m/txns5m above span the last TWO 5m candles (~5–10 min). For thin or
          // dying tokens the last two stored candles can be old and near-empty, so a fading
          // dip-seed reads "0.003% / 1–3 txns" here while it still ticks on DexScreener's m5.
          // Expose (a) candle freshness, (b) a true single-candle 5m read that tracks m5, and
          // (c) a confidence rating — so the UI can label low-trust rows and the scorer can gate
          // clearly-stale data, WITHOUT changing the tuned bounce/buy signal above.
          ...
          // Age from the candle's CLOSE (t + window), not its start. The feed emits only CLOSED
          // 5m candles, so a live token's freshest candle closed 0–5 min ago (age ~0–300s). Measuring
          // from close keeps live tokens from looking old and makes "stale" mean a whole bucket
          // produced no trades — the token actually went quiet.
          ...
          // True 5-min read = the most recent 5m candle alone (open→close) — the closest match to
          // a rolling "last 5 minutes" and to what operators see on DexScreener's m5.
          ...
          // STALE if the freshest candle is older than one full window (token gone quiet);
          // THIN if too few recent trades for the %/ratio to be meaningful; else HIGH.
          ...
          return {
            ...
            // Feed-honesty metadata (see block above): freshness, true single-candle 5m read,
            // and confidence so the dashboard can label low-trust rows and the scorer can gate stale data.
            dataAgeSec,
