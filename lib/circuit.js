...
class CircuitClient {
  ...
  // ── NFT (on-chain Tensor floors / listings / bids / arb, via circuit-node) ──
  // Like call(), but decouples the URL path from the x402 quote key — NFT paths are /api/nft/…
  // while their config keys are hyphenated (nft-floors / nft-collection / nft-arb).
  async _callPaid(path, quoteKey, queryString = '') {
    const full = `${path}${queryString}`;
    if (this._isLocal && this.internalKey) {
      const resp = await this._fetch(full, { 'X-Internal-Key': this.internalKey });
      if (resp.ok) return resp.json();
      if (resp.status !== 402) throw new Error(`API ${resp.status} on ${full}`);
    }
    const first = await this._fetch(full);
    if (first.ok) return first.json();
    if (first.status !== 402) throw new Error(`API ${first.status} on ${full}`);
    if (!this.wallet) throw new Error('Payment required but no wallet configured');
    const quote  = await this.getQuote();
    const epInfo = quote.endpoints?.[quoteKey];
    if (!epInfo) throw new Error(`Unknown endpoint: ${quoteKey}`);
    const txSig = await this._sendCircuitPayment(quote.payment.treasury, BigInt(epInfo.circRaw));
    const paid  = await this._fetch(full, { 'X-Payment-Signature': txSig });
    if (paid.ok) return paid.json();
    const errBody = await paid.json().catch(() => ({}));
    throw new Error(`Paid API call failed ${paid.status}: ${errBody.error ?? ''} (payment txSig: ${txSig.slice(0, 24)})`);
  }

  async nftFloors({ limit = 50, sort = 'listed' } = {}) {
    return this._callPaid('/api/nft/floors', 'nft-floors', `?limit=${limit}&sort=${encodeURIComponent(sort)}`);
  }
  async nftCollection(collection, { listings = 20 } = {}) {
    return this._callPaid(`/api/nft/collection/${collection}`, 'nft-collection', `?listings=${listings}`);
  }
  async nftArb({ minSpreadSol = 0, limit = 50 } = {}) {
    return this._callPaid('/api/nft/arb', 'nft-arb', `?minSpreadSol=${minSpreadSol}&limit=${limit}`);
  }
  async nftAsset(mint) {
    return this._callPaid(`/api/nft/asset/${mint}`, 'nft-asset', '');
  }
  async nftBids(collection, { limit = 50 } = {}) {
    return this._callPaid(`/api/nft/bids/${collection}`, 'nft-bids', `?limit=${limit}`);
  }
  async nftMarket() {
    return this._callPaid('/api/nft/market', 'nft-market', '');
  }
  async nftSearch(q, { limit = 20 } = {}) {
    return this._callPaid('/api/nft/search', 'nft-search', `?q=${encodeURIComponent(q)}&limit=${limit}`);
  }
  async walletNfts(owner, { limit = 200 } = {}) {
    return this._callPaid(`/api/nft/wallet/${owner}`, 'wallet-nfts', `?limit=${limit}`);
  }
  async nftSales(collection, { limit = 50 } = {}) {
    return this._callPaid(`/api/nft/sales/${collection}`, 'nft-sales', `?limit=${limit}`);
  }
