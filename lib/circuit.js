...
// Every state-changing swarm task action is signed by the agent's wallet key so the
// server can prove the caller holds the key behind the address it claims (closes the
// impersonation/IDOR on the task board). The canonical message MUST stay byte-identical
// to the server verifier (circuit-data-api/routes/swarmTasks.js `_swarmAuthMessage`).
const _SWARM_ACTIONS = new Set(['propose', 'claim', 'submit', 'verify', 'cancel', 'abandon', 'subtask']);
const _PKCS8_ED25519_PREFIX = Buffer.from('302e020100300506032b657004220420', 'hex');
function _swarmAuthMessage({ action, address, taskId, submissionId, approved, ts }) {
  const ap = (approved === undefined || approved === null) ? '' : (approved ? '1' : '0');
  return [
    'circuit-swarm-auth-v1',
    action, address || '', taskId || '', submissionId || '', ap, String(ts),
  ].join('\n');
}
function _ed25519SignBase64(secretKey, msg) {
  const seed = Buffer.from(secretKey).subarray(0, 32); // Solana secretKey = seed(32) || pubkey(32)
...
class CircuitClient {
  ...
  async chatCompletion(messages, opts = {}) {
    ...
    let resp = await post();
    if (resp.ok) return { ...extract(await resp.json()), paymentTx: null };
    if (resp.status !== 402) {
    ...
    if (resp.ok) return { ...extract(await resp.json()), paymentTx: txSig };
  ...
  async scanFree(opts = {}) {
    ...
    try {
      ...
      // Process budget: seedMints (dexLosers, placed first) must NOT crowd out the active
      // universe. With ~50 dip-seeds prepended and a flat limit*2 budget + a final slice(0,limit),
      // only ~10 active tokens ever reached the scorer — burying alive dip-reversal setups behind
      // dead crashes. Give the seeds their own headroom PLUS a full active slice, and cover the
      // whole budget with prices/candles. Scoring more candidates only widens selection
      // (gates/safety/buy logic unchanged) — it never over-trades.
      const scanBudget = Math.min(topMints.length, seedMints.length + limit * 2);

      // Step 2: batch price lookup for solReserve → compute liquidityUsd (cover the whole budget)
      const priceMap = {};
      ...
      const candidates = (await Promise.all(topMints.slice(0, scanBudget).map(async (mint) => {
        try {
          // S2 — also pull 1m candles for a FRESH bounce read. The 5m-based bounce spans ~10 min
          // and is often stale by fill time (root cause of ~73%-never-green entries). The 1m
          // series gives a "is the turn still happening right now" signal. Data-only for now:
          // exposed as priceChange1m/bounceFresh and gated opt-in via strategy.requireFreshBounce.
          const [r5m, r1h, r1d, r1m] = await Promise.all([
            ...
            fetch(`${feedBase}/candles/${mint}?window=1m&limit=4`,  { signal: AbortSignal.timeout(3_000) }),
          ]);
          const c5 = r5m.ok ? (await r5m.json()).candles ?? [] : [];
          const c1 = r1h.ok ? (await r1h.json()).candles ?? [] : [];
          const cd = r1d.ok ? (await r1d.json()).candles ?? [] : [];
          const cm = r1m && r1m.ok ? (await r1m.json()).candles ?? [] : [];
          ...
          // Sustained-reversal confirmation. The v0.9.5 speed-up (5min→1min loop + sub-second
          // feed) removed the implicit filter the slow 5-min-lagged source provided: a one-candle
          // dead-cat bounce no longer fades before it triggers a buy. Restore it explicitly — a
          // real turn HOLDS across candles (higher low + advancing close); a dead cat makes a
          // lower low right after the pop. Require the latest 5m candle to hold above the prior
          // candle's low and close at/above the prior close. (~10min of confirmation.)
          ...
          // S2 — fresh 1m bounce read. priceChange1m over the last 2 1m candles (open[-2]→close[-1],
          // same convention as 5m); bounceFresh = the freshest 1m candle is still advancing (higher
          // close than the prior 1m close AND not a red reversal) — the turn is happening NOW, not
          // ~10 min ago. null when 1m data is too sparse to be meaningful (thin token).
          ...
          return {
            ...
            symbol:         '?',
            name:           '?',
            // Scan-time price (SOL per token) — anchor for the S1 live-price confirmation gate
            // and the S4 entry-condition record. priceSol comes from the /prices batch above.
      ...
      // Return the full scored set (bounded by scanBudget). The previous slice(0, limit) cropped
      // in seed-first order, so dead dip-seeds occupied all returned slots and alive setups never
      // reached the scorer. The downstream scorer applies the real dip-reversal selection.
      return { candidates, source: 'price-feed-free' };
    } catch (err) {
  ...
  async feedPriceSol(mint) {
    const feedBase = 'http://127.0.0.1:18941'; // circuit-price-feed, same VPS as scanFree
    try {
   ...
   */
  // Sign a state-changing swarm action with the wallet key. The signed payload binds
  // the action + taskId + (for verify) submissionId/approved + timestamp, so it can't
  // be replayed across actions/tasks or have the approve bit flipped in flight.
  _signTaskBody(endpoint, body) {
    const action = endpoint.split('/').pop();
    if (!_SWARM_ACTIONS.has(action) || !this.wallet?.keypair || !body?.address) return body;
    const ts = Date.now();
    const taskId = body.taskId ?? body.parentTaskId ?? '';
    const msg = _swarmAuthMessage({
      action, address: body.address, taskId,
      submissionId: body.submissionId, approved: body.approved, ts,
    });
    return { ...body, ts, sig: _ed25519SignBase64(this.wallet.keypair.secretKey, msg) };
  }
