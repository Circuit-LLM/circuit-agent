...
async function checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, forceReason = null) {
  ...
  // Block concurrent ticks from double-selling the same position.
  // setInterval fires every 10s without awaiting the async tick, so a sell that
  // takes >10s (Jupiter path can take up to 60s) will overlap with the next tick.
  // Both ticks would pass positions.get() because closePosition() hasn't been called yet.
  ...
  try {
    ...
    if (soldPct < 1) {
      ...
      _inFlight.delete(mint); // partial exit keeps position open — allow next tick to retry
      return;
    }
...
const _inFlight      = new Set(); // mints with a sell currently in progress
...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  const tick = async () => {
    ...
    for (const mint of mints) {
      ...
      // Rug alert — exit immediately regardless of P&L. A rug_alert from the swarm
      // means peer agents have flagged this token as malicious on-chain. Waiting for
      // the 60-minute heartbeat LLM escalation is not safe — move now.
      if (swarmRugAlerts.has(mint)) {
        log('warn', `Swarm rug alert: ${pos.symbol} — exiting immediately`, { mint: mint.slice(0, 8) });
        notify(`🚨 *${pos.symbol}* swarm rug alert — exiting immediately`);
        try {
