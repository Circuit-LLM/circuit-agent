...
async function checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, forceReason = null) {
  ...
  try {
    ...
    notify(
      ...
      `P&L: ${sign}${pnlPct.toFixed(1)}% / ${sign}${realisedPnlSol.toFixed(4)} SOL\n` +
      `Held: ${holdMin.toFixed(0)}min | Peak: +${peak.toFixed(1)}%`
    );
    log('info', 'Position closed', { symbol: preSwapPos.symbol, reason, pnl: pnlPct.toFixed(1) + '%', sol: result.solReceived.toFixed(4) });

    // Report outcome to swarm — makes reputation real and consensus trustworthy
    _reportSwarmOutcome(api, mint, preSwapPos.symbol, pnlPct, realisedPnlSol, holdMin, cfg)
    ...
    _broadcastSellSignal(api, mint, preSwapPos.symbol, pnlPct, reason, cfg)
      .catch(() => {});

    // Reinvest a slice of profit into CIRCUIT — the agent stays alive by being profitable
    if (realisedPnlSol > 0) {
    ...
    log('error', 'Sell failed', { symbol: preSwapPos.symbol, error: err.message });
