...
async function checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, forceReason = null) {
  ...
  try {
    ...
    _clearSellFail(mint);
    // For partial sells compare against the proportional cost basis, not the full
    // investment — otherwise a 50% sell always looks like a loss against 100% spent.
    const costBasis    = preSwapPos.solSpent * soldPct;
