...
//      Misses: /warm pre-populates Redis via Jupiter (30s TTL)
...
//    token in a 10-second window — catches phantom prices from stale Redis entries
//    or raced pool records at any point in the hold, including after the 5-min
//    entry guard expires. Pump.fun bonding curve tokens with incomplete=true pool
//    records are most vulnerable to this class of error.
...
const TICK_SPIKE_MAX_SKIPS     = 3;    // after this many consecutive >5× skips, accept the
                                       // price as real — a persistent "spike" is the live
                                       // baseline (or a stale seed), not a transient phantom.
                                       // Without this, monitoring freezes and the stop-loss
                                       // and max-hold checks never run (see -20% / 57min hold).
// Phantom-gain guard: a feed price 1.5×–5× the last reading slips under TICK_SPIKE_MULTIPLE
// but can be a sustained mispricing (e.g. +90% for a minute). Left unchecked it corrupts the
// peak and trips the trailing stop on a price that was never tradeable. When the feed implies
// a large gain via a big jump, cross-check Jupiter before trusting it.
const PHANTOM_GAIN_PCT       = 25;   // only validate when feed implies >= +25% gain
const PHANTOM_JUMP_MULTIPLE  = 1.5;  // ...and the price jumped > 1.5× the last valid reading
const PHANTOM_GAIN_TOLERANCE = 15;   // reject if feed is > 15pp more optimistic than Jupiter
// Stale-peak guard: a feed price unchanged for this many consecutive ticks is frozen (the feed
// stopped updating), not a real flat. A peak set by such a value must not be trailed from once
// the feed unfreezes to a lower price (observed: +2.9% pinned 60+ ticks → snap to -0.2% →
// premature trailing-stop exit at a small loss).
const STALE_PEAK_TICKS       = 5;
// Absolute phantom ceiling: a gain this extreme in a short memecoin hold is a feed glitch, not a
// real position (take-profit fires at +25% long before any real move gets here). The multiplicative
// and single-tick-jump guards miss GRADUAL phantom inflation — observed peaks of +6573% and +98998%
// crept up over many sub-threshold ticks, corrupting peak tracking and the trade record. This is a
// hard backstop: such a pnl never updates the peak and never triggers a gain exit. Protective exits
// (max-hold / stop / dead-money) still run, so a stuck-phantom position can't get orphaned.
const MAX_PLAUSIBLE_GAIN_PCT = 800;

const _lastGoodPrice = new Map();      // mint → last validated price
const _tickSpikeSkips = new Map();     // mint → consecutive tick-spike skip count
const _lastTickPrice = new Map();      // mint → last raw feed price (staleness detection)
const _stalePriceTicks = new Map();    // mint → consecutive unchanged-price tick count

// Swarm coordination feed: the FREE public feed (no key/payment) is refreshed at most every
// _SWARM_FEED_TTL_MS. Signals live ~10 min, so a 30s cache is plenty fresh and bounds load on
// the shared free endpoint regardless of tick rate. Cache the RAW signals (not the filtered exit
// sets) because the held-mint filter depends on the current position set each tick.
const _SWARM_FEED_TTL_MS = 30_000;
let _swarmFeedCache = { ts: 0, signals: [] };
...
async function checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, forceReason = null) {
  ...
  // Guard against non-finite prices from the price-feed (e.g. division by near-zero reserves).
  ...
  // Phantom prices from stale pool records can arrive after the 5-min entry guard expires.
  // No real token moves 5× in one 10-second poll cycle.
  //
  ...
  if (!forceReason && _prevPrice > 0 && currentPrice > _prevPrice * TICK_SPIKE_MULTIPLE) {
    const skips = (_tickSpikeSkips.get(mint) ?? 0) + 1;
    if (skips < TICK_SPIKE_MAX_SKIPS) {
      _tickSpikeSkips.set(mint, skips);
      log('warn', `${livePos.symbol} spike-guard(tick): ${currentPrice.toExponential(2)} vs prev ${_prevPrice.toExponential(2)} (${(currentPrice / _prevPrice).toFixed(0)}×) at ${holdMin.toFixed(1)}min — skipping tick (${skips}/${TICK_SPIKE_MAX_SKIPS})`);
      return;
    }
    // Persisted across TICK_SPIKE_MAX_SKIPS ticks → this is the live price (or the seed
    // baseline was stale), not a transient phantom. Accept it so stop-loss / max-hold resume
    // instead of skipping ticks forever (the cause of the unmonitored -20% / 57min hold).
    log('warn', `${livePos.symbol} spike-guard(tick): persisted ${skips} ticks — accepting ${currentPrice.toExponential(2)} as the live baseline`);
  }
  _tickSpikeSkips.delete(mint);

  // Phantom-gain guard — see PHANTOM_GAIN_* constants. Catches sustained mispricings that pass
  // the multiplicative spike guards and would otherwise inflate the peak / trip trailing stops.
  if (!forceReason && _feedBase && pnlPct >= PHANTOM_GAIN_PCT && _prevPrice > 0 && currentPrice > _prevPrice * PHANTOM_JUMP_MULTIPLE) {
    const jupSolPrice = await _jupiterSpotCheck(mint);
    if (jupSolPrice !== null) {
      const jupPnlPct = ((jupSolPrice * uiAmount - livePos.solSpent) / livePos.solSpent) * 100;
      if (pnlPct - jupPnlPct > PHANTOM_GAIN_TOLERANCE) {
        log('warn', `${livePos.symbol} phantom-gain guard: feed ${pnlPct.toFixed(1)}% but Jupiter ${jupPnlPct.toFixed(1)}% — rejecting feed price, not updating peak`);
        _lastGoodPrice.set(mint, jupSolPrice); // anchor next tick's guards to the verified price
        return;
      }
    }
  }
  ...
  // Stale-peak guard — see STALE_PEAK_TICKS. A frozen feed price can set an inflated peak that
  // was never tradeable; when the feed unfreezes lower, the trailing stop fires on a phantom
  // high. On the first fresh, lower tick after a frozen stretch, re-anchor the peak to the live
  // price. Asymmetric: only ever lowers a stale peak, never blocks a real new high.
  const _lastPx     = _lastTickPrice.get(mint);
  const priceFrozen = _lastPx != null && Math.abs(currentPrice - _lastPx) <= _lastPx * 1e-6;
  const priorStale  = _stalePriceTicks.get(mint) ?? 0;
  if (priceFrozen) {
    _stalePriceTicks.set(mint, priorStale + 1);
  } else {
    if (priorStale >= STALE_PEAK_TICKS && _peakCache.has(mint) && pnlPct < _peakCache.get(mint)) {
      log('warn', `${livePos.symbol} stale-peak reset: peak ${_peakCache.get(mint).toFixed(1)}% was frozen ${priorStale} ticks — re-anchoring to live ${pnlPct.toFixed(1)}%`);
      _peakCache.set(mint, pnlPct);
    }
    _stalePriceTicks.set(mint, 0);
  }
  _lastTickPrice.set(mint, currentPrice);
  ...
  // Phantom-high backstop: an implausible gain never updates the peak (prevents the +6573%/
  // +98998% peak corruption from gradual phantom inflation). Protective exits below still run.
  const phantomHigh  = pnlPct > MAX_PLAUSIBLE_GAIN_PCT;
  ...
  if (!phantomHigh && pnlPct > cachedPeak) _peakCache.set(mint, pnlPct);
  else if (isFirstTick)                    _peakCache.set(mint, cachedPeak);
  const peak = phantomHigh ? cachedPeak : Math.max(pnlPct, cachedPeak);
  if (phantomHigh) log('warn', `${livePos.symbol} phantom-high ignored: feed pnl ${pnlPct.toFixed(0)}% > ${MAX_PLAUSIBLE_GAIN_PCT}% ceiling — peak held at ${peak.toFixed(1)}%`);

  // Momentum-aware max-hold extension. max-hold is a timeout for positions that never developed;
  // once a position has developed a real peak (trailing armed) it is a live winner, and cutting it
  // flat at the deadline throws away the run the trailing stop exists to capture (this is how e.g.
  // `world` was clipped at +3.7% then ran to +25%). So while armed we let the trailing stop own the
  // exit past maxHold, up to an absolute hard cap. Crucially the trail is TIGHTENED during the
  // extension (narrower distance + higher lock): backtest showed the naive "just extend on the same
  // wide trail" is a wash — extended winners round-trip back to the loose floor — whereas tightening
  // it turns the extension net-positive without degrading win-rate/median. Opt-in and inert by
  // default: hardCap falls back to maxHoldMinutes (no extension) unless maxHoldHardCapMinutes is set.
  const hardCapMinutes       = Math.max(s.maxHoldHardCapMinutes ?? maxHoldMinutes, maxHoldMinutes);
  const inMaxHoldExtension   = holdMin >= maxHoldMinutes && peak >= trailingActivate && holdMin < hardCapMinutes;
  const effTrailingDistance  = inMaxHoldExtension ? (s.extTrailingStopDistancePct ?? trailingDistance) : trailingDistance;

  // Trailing stop threshold (only active once peak >= trailingActivate)
  let trailingThreshold = peak >= trailingActivate ? peak - effTrailingDistance : null;
  // Profit lock — the dominant exit failure was winners round-tripping to ~breakeven (a +4.5%
  // peak realized −0.1% via a wide give-back). Once a position has reached a real peak, ratchet
  // the exit floor up to a locked fraction of that peak so a winner exits GREEN instead of giving
  // it all back. This only ever RAISES the threshold (locks more) — it never loosens the stop, and
  // a true runner is still handled by the tighter peak−distance trail above it. Tunable per agent.
  const lockTrigger  = s.lockProfitTriggerPct ?? 4;   // peak must reach +4% to arm the lock
  const lockFraction = inMaxHoldExtension
    ? (s.extLockProfitFraction ?? s.lockProfitFraction ?? 0.5)   // tighter lock during the extension
    : (s.lockProfitFraction ?? 0.5);
  if (peak >= lockTrigger) {
    const lockFloor = peak * lockFraction;
    trailingThreshold = trailingThreshold === null ? lockFloor : Math.max(trailingThreshold, lockFloor);
  }
  ...
  // phantom exits from post-buy price data not yet reflecting the actual pool state.
  // minHoldBeforeSlMinutes: SL grace window after entry. Was 2 min when unindexed PumpSwap
  // tokens fell back to slower external APIs. PumpSwap is now indexed via gRPC so prices
  // are sub-second and stale-price false stop-outs are no longer a risk. Default 0.
  ...
  } else if (!phantomHigh && pnlPct >= takeProfitPct && holdMin >= minHoldBeforeTp) {
    reason = 'take-profit';
  } else if (_isDeadMoney(mint, holdMin, s, s.positionCheckMs ?? 10_000)) {
    reason = 'dead-money';
  } else if (holdMin >= maxHoldMinutes) {
    // Armed winner past the deadline → let the (now tightened) trailing stop own the exit instead of
    // cutting flat, bounded by hardCapMinutes. A position that never armed still times out here.
    if (inMaxHoldExtension) {
      log('info', `${livePos.symbol} max-hold ${holdMin.toFixed(0)}min but armed (peak ${peak.toFixed(1)}%, now ${pnlPct >= 0 ? '+' : ''}${pnlPct.toFixed(1)}%) — extending to ${hardCapMinutes}min cap, tightened trail owns exit`);
    } else {
      reason = 'max-hold';
    }
  }
  ...
  // Jupiter spot-check for stop-loss and trailing-stop.
  // The price-feed resolution chain has multiple paths (bonding curve records, RPC, cache)
  // and any of them can produce a phantom price under specific race conditions. Before
  // committing to a hard exit, call Jupiter (~150-300ms) for an independent on-chain price.
  // If Jupiter shows we are NOT at stop-loss: defer one tick and update _lastGoodPrice to
  // the Jupiter price so Guard 2 (tick-spike) uses it as the new baseline.
  // forceReason (swarm-exit, lp-drain, swarm-rug) bypasses this check — speed is critical.
  if ((reason === 'stop-loss' || reason === 'trailing-stop' || reason === 'take-profit') && !forceReason && _feedBase) {
    const jupSolPrice = await _jupiterSpotCheck(mint);
    if (jupSolPrice !== null) {
      const jupSolValue = jupSolPrice * uiAmount;
      const jupPnlPct   = ((jupSolValue - livePos.solSpent) / livePos.solSpent) * 100;
      if (reason === 'take-profit') {
        // Defer a phantom take-profit: the feed says we hit +TP but Jupiter shows we are well
        // below it. Selling on the phantom realizes the real (low) price — the "+2.2% take-profit"
        // signature that cuts real winners short. Only let a real TP fire when both sources agree.
        if (jupPnlPct < takeProfitPct - PHANTOM_GAIN_TOLERANCE) {
          log('warn', `${livePos.symbol} tp-guard: feed ${pnlPct.toFixed(1)}% but Jupiter ${jupPnlPct.toFixed(1)}% — deferring take-profit (phantom high)`);
          _lastGoodPrice.set(mint, jupSolPrice);
          return;
        }
      } else if (reason === 'trailing-stop') {
        // A trailing stop fires because price pulled BACK below peak−distance. The right phantom
        // check is "does Jupiter confirm the pullback?" — i.e. is Jupiter also at/under the trail
        // threshold? Asking whether Jupiter is above stopLossPct (the old shared branch) is
        // meaningless for a profitable position: a +40% winner is always above a −4% stop, so the
        // trail deferred every tick and NO winner could ever trail-exit — they all ran to max-hold
        // instead (0 trailing exits 07-19→07-22 vs 17 max-hold; Jimothy trailed 20min at +40%).
        // Defer only when Jupiter says we are still clearly ABOVE the trail (the feed dipped but
        // the real price did not). Tolerance is trail-scaled, NOT the 15pp phantom-gain constant:
        // trail thresholds sit only 2-4pp below peak, so a 15pp margin would make this branch
        // unreachable and kill the phantom-dip protection. A few pp cleanly separates normal
        // feed/Jupiter agreement (~0.1-0.5pp, seen live) from a genuine phantom dip.
        const TRAIL_CONFIRM_TOL_PCT = 3;
        if (jupPnlPct > trailingThreshold + TRAIL_CONFIRM_TOL_PCT) {
          log('warn', `${livePos.symbol} trail-guard: feed ${pnlPct.toFixed(1)}% but Jupiter ${jupPnlPct.toFixed(1)}% > trail ${trailingThreshold.toFixed(1)}% — deferring trailing-stop (phantom dip)`);
          _lastGoodPrice.set(mint, jupSolPrice);
          return;
        }
        log('warn', `${livePos.symbol} TRAIL-EXIT: feed ${pnlPct.toFixed(1)}% (peak ${peak.toFixed(1)}%, trail ${trailingThreshold.toFixed(1)}%) — Jupiter ${jupPnlPct.toFixed(1)}% confirms, exiting`);
        // fall through to the exit
      } else if (jupPnlPct > stopLossPct) {
        // S5 — hard-stop floor: never defer once catastrophically underwater. The defer exists to
        // dodge phantom single-tick stops near the threshold; below the floor the loss is deep and
        // real, and waiting a tick on a fast dump is exactly how −5% stops realized −8% to −11%
        // (e.g. HermesWorld −10.6% in 1 min). The actual sell still executes at the real on-chain
        // price, so exiting on a deep feed reading is safe even if Jupiter momentarily disagrees.
        // Tunable: strategy.hardStopMultiple (× stopLossPct; default 1.6 ⇒ −6% stop floors at −9.6%).
        const hardStopPct = stopLossPct * (s.hardStopMultiple ?? 1.6);
        if (pnlPct > hardStopPct) {
          log('warn', `${livePos.symbol} stop-guard: feed=${currentPrice.toExponential(3)} (${pnlPct.toFixed(1)}%) but Jupiter=${jupSolPrice.toExponential(3)} (${jupPnlPct.toFixed(1)}%) — deferring ${reason}`, {
            holdMin: holdMin.toFixed(1),
          });
          _lastGoodPrice.set(mint, jupSolPrice); // anchor next tick's Guard 2 to the verified price
          return;
        }
        log('warn', `${livePos.symbol} HARD-STOP: feed ${pnlPct.toFixed(1)}% ≤ floor ${hardStopPct.toFixed(1)}% — exiting now (Jupiter ${jupPnlPct.toFixed(1)}%, no defer)`);
        // fall through to the exit
      }
      // Jupiter confirms → proceed immediately with the exit
    }
    // Jupiter unavailable → fail open, proceed with the exit at current speed
  }
  ...
  try {
    // Use live on-chain balance so any dust accumulated from prior partial sells is
    // included. NEVER fall back to preSwapPos.tokenAmount: that value is the Jupiter
    // QUOTE, not the realised fill (swap.js buy() reads result.outputAmount, which
    // neither the Jito path nor Ultra /execute returns, so it lands on order.outAmount).
    // It therefore always overshoots the true balance — measured at +5,279 raw units on
    // a Jito entry and +1 on a Jupiter entry. Selling 100% of an overshoot returns
    // "Insufficient funds", which the partial-sell ladder below misclassifies as a
    // liquidity error and answers by halving. The position stays open, the ladder
    // restarts at 100% next tick, and the position is shredded geometrically
    // (HBULL 2026-08-03: RPC 503 → 17 halvings → -0.0094 SOL on a flat position).
    let rawAmount, rawAmountBig;
    const liveBal = await swap.getTokenBalance(mint);
    if (liveBal.ok === false) {
      // Balance UNKNOWN (RPC failure) — not zero. Defer; _sellCooledDown throttles the
      // retry (30s/60s/120s cap) and the reason is recomputed next tick, so nothing is
      // stranded: the exit simply happens when the RPC answers again.
      _recordSellFail(mint);
      releaseSellLock(mint);
      log('warn', `${preSwapPos.symbol} exit deferred — cannot verify on-chain balance (RPC); will retry`, { reason });
      return;
    }
    if (liveBal.rawAmount === 0n) {
      // Confirmed empty on-chain: the tokens are already gone. Close with no exit data
      // so no invented P&L reaches trade_history.
      positions.closePosition(mint, {});
      releaseSellLock(mint);
      log('warn', `${preSwapPos.symbol} position empty on-chain — closed, no trade recorded`, { reason });
      return;
    }
    rawAmountBig = liveBal.rawAmount;
    // Keep the full-precision BigInt for the on-chain sell (no dust on huge positions); rawAmount is a
    // Number used only for the approximate local P&L estimate + the slippage-check URL.
    rawAmount = Number(rawAmountBig);

    // Pre-sell slippage gate for take-profit only.
    // Phantom TPs can happen when a stale pool record or raced bonding curve write
    // shows an inflated price while the pool has already reverted. As a final safety
    // check, ask the price feed to estimate actual SOL out from reserve math.
    ...
    for (const pct of PARTIAL_PCTS) {
      try {
        result = await swap.sell(mint, rawAmountBig, pct); // precise on-chain amount
        soldPct = pct;
    ...
    if (soldPct < 1) {
      ...
      // Abandon a remainder that costs more to sell than it is worth. Each retry pays a
      // full round-trip fee (~0.0004 SOL), so without a floor a position that keeps
      // failing at 100% is halved every tick until it is dust — 13 of the 17 sells in
      // the HBULL 2026-08-03 cascade were individually SOL-negative. Value the remainder
      // at the price this tick already fetched.
      const remainingSol = (remaining / Math.pow(10, preSwapPos.tokenDecimals ?? 6)) * currentPrice;
      const dustFloorSol = cfg?.strategy?.dustFloorSol ?? 0.0008; // ~2x a round-trip fee
      if (remainingSol < dustFloorSol) {
        positions.closePosition(mint, {});
        log('warn', `${preSwapPos.symbol} remainder ${remainingSol.toFixed(6)} SOL below dust floor ${dustFloorSol} — abandoning, not chasing`, { remaining });
        notify(`⚠️ *${preSwapPos.symbol}* partial exit — remainder (${remainingSol.toFixed(5)} SOL) worth less than the fee to sell it; abandoned`);
        _tpConfirm.delete(mint);
        releaseSellLock(mint);
        return;
      }
    ...
    _tickSpikeSkips.delete(mint);
    _lastTickPrice.delete(mint);
    _stalePriceTicks.delete(mint);
    ...
    const exitData = {
      ...
      reason,
      txSig:       result.txSig,
      execCosts:   result.execCosts ?? null, // sell-leg tip + network fee → netPnlSol in the trade record
    };
    ...
    // Wallet-true P&L: gross minus both legs' tips + network fees (when booked).
    // Gross remains the price-tracking basis; NET drives reputation, reinvest and stats.
    const feesSolPaid    = (preSwapPos.entryCostsSol ?? 0) + (result.execCosts?.costSol ?? 0);
    const netRealisedSol = (preSwapPos.entryCostsSol != null || result.execCosts?.costSol != null)
      ? realisedPnlSol - feesSolPaid : null;
    ...
    notify(
      ...
      (netRealisedSol != null ? `\nNet: ${netRealisedSol >= 0 ? '+' : ''}${netRealisedSol.toFixed(4)} SOL (fees ${feesSolPaid.toFixed(4)})` : '') +
    ...
    // Dispatch webhook on trade_closed (fire-and-forget, never blocks)
    webhooks.dispatchWebhook('trade_closed', {
      symbol: preSwapPos.symbol,
      mint,
      entryPrice: preSwapPos.entryPrice,
      exitPrice: currentPrice,
      entryTime: preSwapPos.entryTime,
      exitTime: exitData.exitTime,
      holdMinutes: holdMin,
      pnlSol: realisedPnlSol,
      pnlPct: realisedPnlPct,
      netPnlSol: netRealisedSol,
      reason,
      txSig: result.txSig,
      peak: peak,
    }, cfg).catch(() => {});
    ...
    if (!swap?.paperMode) {
      _reportSwarmOutcome(api, mint, preSwapPos.symbol, realisedPnlPct, realisedPnlSol, netRealisedSol, holdMin, cfg)
    ...
    // Reinvest from NET profit — don't buy CIRC with SOL that fees already consumed.
    const reinvestBaseSol = netRealisedSol ?? realisedPnlSol;
    if (reinvestBaseSol > 0 && !swap?.paperMode) {
      reinvestProfit({ pnlSol: reinvestBaseSol, symbol: preSwapPos.symbol, swap, wallet, cfg, notify })
...
async function _getSwarmSignals(mints, cfg, api) {
  ...
  try {
    // Read the FREE public swarm feed (no internal key, no x402 — the same tier any external
    // agent uses). feed-public filters by `exclude`, not `type`, so swarmFeedPublic() excludes
    // the high-volume noise types (scan_quality/agent_profile/buy_signal/task_submitted) to keep
    // sell_signal/rug_alert in the window. Throttled to _SWARM_FEED_TTL_MS to bound shared load.
    const now = Date.now();
    if (now - _swarmFeedCache.ts >= _SWARM_FEED_TTL_MS) {
      const feed = await api.swarmFeedPublic({ limit: 200 });
      _swarmFeedCache = { ts: now, signals: feed.signals ?? [] };
    }
    const signals  = _swarmFeedCache.signals;
...
// ── Holder-exodus flags (written by lib/watches.js holder guard) ─────────────
const HOLDER_FLAG_FILE = path.join(__dirname, '../data/holder_exodus.json');
let _holderFlagCache = { mtime: 0, data: {} };
function _holderExodusFlags() {
  try {
    const st = fs.statSync(HOLDER_FLAG_FILE);
    if (st.mtimeMs !== _holderFlagCache.mtime) {
      _holderFlagCache = { mtime: st.mtimeMs, data: JSON.parse(fs.readFileSync(HOLDER_FLAG_FILE, 'utf8')) };
    }
  } catch { _holderFlagCache = { mtime: 0, data: {} }; }
  return _holderFlagCache.data;
}
function _markHolderFlagHandled(mint) {
  try {
    const data = _holderExodusFlags();
    if (data[mint]) { data[mint].handled = true; fs.writeFileSync(HOLDER_FLAG_FILE, JSON.stringify(data, null, 2)); _holderFlagCache.mtime = 0; }
  } catch { /* best-effort */ }
}

// ── Swarm outcome reporting ───────────────────────────────────────────────────

async function _reportSwarmOutcome(api, mint, symbol, pnlPct, pnlSol, netPnlSol, holdMinutes, cfg) {
  const identity = loadIdentity();
  if (!identity.agentId && !identity.address) return;

  // Verdict on wallet-true P&L when fees were booked: a gross "win" that fees turned
  // into a loss must not inflate this agent's reputation (or the swarm's win rates).
  await api.swarmOutcome({
    agentId:  identity.agentId,
    address:  identity.address,
    mint, symbol, pnlPct, pnlSol, netPnlSol, holdMinutes,
    verdict:  (netPnlSol ?? pnlSol) > 0 ? 'win' : 'loss',
  });
...
// Fast Jupiter spot-check: independent price confirmation in ~150-300ms.
// Used to cross-verify stop-loss and trailing-stop triggers before executing a sell.
// Returns priceSol (SOL per 1 UI token) or null if Jupiter is unavailable.
//
// Fetches the token AND wSOL in one call and divides, so priceSol is derived directly
// (the USD cancels) — no dependency on an external SOL/USD oracle. Uses lite-api.jup.ag,
// Jupiter's free host; api.jup.ag requires a paid key and returns 429 unauthenticated.
async function _jupiterSpotCheck(mint) {
  try {
    const resp = await fetch(
      `https://lite-api.jup.ag/price/v3?ids=${mint},${_SOL_MINT}`,
      { signal: AbortSignal.timeout(3_000) },
    );
    if (!resp.ok) return null;
    const data     = await resp.json();
    const entry    = data?.[mint];
    const solEntry = data?.[_SOL_MINT];
    if (!entry || !solEntry) return null;
    const priceUsd = parseFloat(entry.usdPrice ?? entry.price);
    const solUsd   = parseFloat(solEntry.usdPrice ?? solEntry.price);
    if (!priceUsd || !isFinite(priceUsd) || !solUsd || !isFinite(solUsd)) return null;
    return priceUsd / solUsd;
  } catch { return null; }
}

function _deriveFeedBase(cfg) {
  // Shared resolver (lib/feedBase.js) — the scanner (lib/circuit.js) resolves the feed the same
  // way, so the monitor and scanner can never drift onto different price-feed hosts.
  return deriveFeedBase({ priceFeedUrl: cfg.priceFeedUrl, baseUrl: cfg.api?.baseUrl });
}
...
async function _fetchPriceFeedPrices(mints) {
  ...
  for (const mint of mints) {
    ...
    if (r?.priceSol > 0) {
      ...
      feedData[mint] = { priceSol: r.priceSol, solReserve: r.solReserve ?? null, source: r.source ?? null, priceAgeMs: r.ageMs ?? null }; // feed returns `ageMs` on /prices
    }
  }
...
// that stop returning price data from any source — without this, positions sit indefinitely.
const _priceSkipCount = new Map();
const PRICE_STALE_THRESHOLD = 30;
// A feed price older than this is treated as not-yet-confirmed: it gets re-checked against the
// independent Jupiter source before any exit math. The price-feed serves a value until its Redis key
// expires (~120s), so a CPMM/PumpSwap pool whose vault updates we stop receiving leaves the LAST price
// FROZEN — during which a real dump is invisible and the stop-loss can't fire. 45s is well inside that
// window and far above the few-second refresh of an actively-traded token, so it won't false-trigger.
const STALE_FEED_AGE_MS = 45_000;
...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  const tick = async () => {
    ...
    // Misses are handled server-side: price-feed falls back to Jupiter and caches
    // the result in Redis (30s TTL), so no agent ever calls external APIs directly.
    // For freshly bought tokens, auto-scanner calls /warm immediately after the buy
    // so the first monitor tick is already a Redis hit.
    let priceMap = {};
    let feedData = {};
    ...
    if (_feedBase) {
      try {
        ...
        if (misses.length) {
          // Warm misses: price-feed resolves via Jupiter + caches in Redis.
    ...
    // Update per-mint reserve history (only available from price-feed, not x402 fallback).
    ...
    for (const mint of mints) {
      ...
      let   currentPrice = priceData?.priceNative ?? null;

      // Staleness guard (the one piece of data the feed returns but the agent never read): if the
      // feed's own age says this price is stale, don't trust it for exit math — confirm against
      // Jupiter (independent, ~200ms) and use that fresh value. A quiet token just gets re-confirmed
      // (same price, no harm); a token that actually moved while its vault updates were missed gets
      // its TRUE price, so the stop-loss / trailing-stop fire correctly instead of on a frozen value.
      const _feedAgeMs = feedData[mint]?.priceAgeMs;
      if (currentPrice !== null && _feedAgeMs != null && _feedAgeMs > STALE_FEED_AGE_MS) {
        const _fresh = await _jupiterSpotCheck(mint);
        if (_fresh && isFinite(_fresh) && _fresh > 0) {
          if (Math.abs(_fresh - currentPrice) / currentPrice > 0.02) {
            log('warn', `${pos.symbol} feed price stale (${Math.round(_feedAgeMs / 1000)}s) — using fresh Jupiter price`,
              { mint: mint.slice(0, 8), feed: currentPrice, jupiter: _fresh });
          }
          currentPrice = _fresh;
        }
        // Jupiter unavailable → keep the feed price (still the best available); the existing
        // null-skip emergency exit + the feed's 120s TTL remain as backstops.
      }
      ...
      // Whale exit: the holder-exodus guard (lib/watches.js, 5-min tick) flags positions
      // whose entry-time top holders dumped >= holderExitDropPct. Only acts when the
      // operator opted in via strategy.holderExodusExit === 'exit' (default is alert-only).
      const whaleFlag = _holderExodusFlags()[mint];
      if (whaleFlag?.action === 'exit' && !whaleFlag.handled) {
        log('warn', `${pos.symbol} whale exit: top holder dumped ${whaleFlag.dropPct}% — exiting`, { mint: mint.slice(0, 8) });
        notify(`🐋 *${pos.symbol}* top holder dumped ${whaleFlag.dropPct}% of their entry-time stack — exiting (whale-exit)`);
        try {
          await checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, 'whale-exit');
          _markHolderFlagHandled(mint);
        } catch (err) {
          log('error', 'Whale-exit error', { mint: mint.slice(0, 8), error: err.message });
        }
        continue;
      }
      ...
      // LP-drain emergency sell is OFF by default: populating reserves (P1) makes this detector
      // live for many more tokens, so it stays gated behind strategy.lpDrainExit until deliberately
      // enabled + monitored. See circuit-price-feed/docs/SCAN_LIQUIDITY_FIX_PLAN.md
      if (cfg.strategy?.lpDrainExit === true && feedInfo?.solReserve != null && _isLpDrain(mint, feedInfo.source)) {
        const hist = _reserveHist.get(mint) ?? [];
    ...
    for (const m of _peakCache.keys()) {
      if (!held[m]) { _peakCache.delete(m); _tpConfirm.delete(m); _pnlHistory.delete(m); _lastGoodPrice.delete(m); _tickSpikeSkips.delete(m); _lastTickPrice.delete(m); _stalePriceTicks.delete(m); }
    }
  };
