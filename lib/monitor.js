...
let _lastSolUsd  = 155;  // SOL/USD rate — updated from price-feed batch response, seeded conservatively
...
async function _fetchPriceFeedPrices(mints) {
  ...
  // Cache SOL/USD for Jupiter spot-checks (Jupiter returns USD, we need SOL).
  if (data.solUsd > 0) _lastSolUsd = data.solUsd;
  const priceMap = {};
