...
async function checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, forceReason = null) {
  ...
  // Take-profit is a safety ceiling (set to 100% in config) — CIRC reinvestment
  // still fires on any profitable exit regardless of reason.
  let reason = null;
  ...
  try {
    ...
    if (soldPct < 1) {
      ...
      return; // exit without closing — monitor retries on next tick
    }
...
async function _fetchDexscreenerPrices(mints) {
  ...
  for (const mint of mints) {
    ...
    // Prefer SOL-quoted pairs — priceNative on a USDC/USDT pair is a USD value,
    // not SOL, and would corrupt the SOL-denominated P&L calculation.
    const solPairs = tokenPairs.filter(p => p.quoteToken?.address === SOL_MINT);
    const candidates = solPairs.length ? solPairs : tokenPairs;
