...
// If the feed returns a price >SPIKE_GUARD_MULTIPLE times the entry price within
// the first SPIKE_GUARD_MAX_HOLD_MIN minutes, the data is almost certainly wrong
// (e.g. the feed returned USD instead of SOL for this mint, or a stale/corrupt
// Redis entry). We skip the entire tick so a bad reading cannot corrupt the peak
// cache or fire a phantom take-profit. After the hold window, the slippage gate
// in the TP path handles the same class of error.
const SPIKE_GUARD_MULTIPLE     = 20;   // 20× entry price
const SPIKE_GUARD_MAX_HOLD_MIN = 5;    // only guard within first 5 min
...
async function checkPosition(mint, pos, currentPrice, swap, wallet, cfg, notify, api, forceReason = null) {
  ...
  // Price-spike guard: reject implausible readings early in the hold window.
  // Protects the peak cache and all exit logic from corrupted feed data.
  if (
    !forceReason &&
    ...
    log('warn', `${livePos.symbol} price-spike guard: ${currentPrice.toExponential(2)} vs entry ${livePos.entryPrice.toExponential(2)} (${(currentPrice / livePos.entryPrice).toFixed(0)}×) at ${holdMin.toFixed(1)}min — skipping tick`);
    return;
  }
  ...
  // minHoldBeforeSlMinutes: SL is suppressed for this many minutes after entry to prevent
  // false stop-outs from stale DexScreener prices on freshly bought pumpfun/unindexed tokens.
  // The scan cache (30s) + DexScreener propagation lag (~15s) can show the pre-bounce dip price
  // for up to ~90s after entry; a 2-minute SL grace window covers this safely.
  const minHoldBeforeTp = s.minHoldBeforeTpMinutes ?? 3;
  const minHoldBeforeSl = s.minHoldBeforeSlMinutes ?? 2;
  let reason = null;
...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  const tick = async () => {
    ...
    // ── 2. x402 fallback — only when price-feed service is unreachable ────────
    if (feedDown) {
      try {
        ...
        log('info', `Prices fetched (x402 fallback) for ${Object.keys(raw).length}/${mints.length} position(s)`);
      } catch (err) {
