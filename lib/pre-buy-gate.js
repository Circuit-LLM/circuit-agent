...
async function check(candidate, strategy, openPositions) {
  ...
  try {
    ...
    // On timeout or error, approve to avoid missing opportunities — but flag it so the
    // caller can notify the user that selective-mode oversight was temporarily bypassed.
    log('warn', `Gate error — auto-approving`, {
      symbol: candidate.symbol ?? candidate.mint?.slice(0, 8),
      score:  candidate._score,
      error:  err.message,
    });
    return { approved: true, timedOut: true, reasoning: `Gate error: ${err.message}` };
  } finally {
    clearTimeout(timer);
  }
}
