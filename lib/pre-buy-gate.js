...
//   - Hard timeout: 20s. FAIL-CLOSED on timeout/error: the candidate is rejected and
//     the next scan (≤60s) retries with a live gate. timedOut:true marks the transient case.
//   - Only a MISSING key bypasses the gate (approved) — selective mode without an LLM
//     configured degrades to active-mode behavior rather than halting all buys.
...
function buildCandidateBrief(candidate, strategy, openPositions, maxOpen = 3) {
  ...
  const lines = [
    ...
    `Open positions: ${openPositions}/${maxOpen}`,
    `Session mode: ${strategy.mode} | Goal: ${strategy.sessionGoal}`,
  ];

  if (_breakdown) {
    const bd = Object.entries(_breakdown)
      .map(([k, v]) => `${k}:${typeof v === 'number' ? v.toFixed(1) : typeof v === 'object' ? JSON.stringify(v) : v}`)
      .join(' ');
...
async function check(candidate, strategy, openPositions) {
  ...
  const brief = buildCandidateBrief(candidate, strategy, openPositions, settings.strategy?.maxOpenPositions ?? 3);
  ...
  try {
    ...
    // Fail closed: a selective-mode gate that approves on error is not a gate.
    // If the LLM API is unavailable (timeout, 429, network failure), reject this
    // candidate and let the scanner retry on the next cycle with a live gate response.
    // timedOut: true lets callers distinguish a transient bypass from a hard rejection.
    log('warn', `Gate error — rejecting (fail-closed)`, {
      symbol: candidate.symbol ?? candidate.mint?.slice(0, 8),
      score:  candidate._score,
      error:  err.message,
    });
    return { approved: false, timedOut: true, reasoning: `Gate unavailable: ${err.message}` };
  } finally {
    clearTimeout(timer);
  }
}
