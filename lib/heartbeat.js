...
const dcaExecutor        = require('./dca-executor');
...
async function reportToRegistry(apiBase, internalKey, agentId, address, keypair) {
  if (!apiBase || !agentId) return;
  try {
    const held    = positions.getAll();
    const history = positions.getTradeHistory(100, 30);
    const wins    = history.filter(t => (t.netPnlSol ?? t.pnlSol ?? t.pnlPct ?? 0) > 0).length;
    ...
    let body = {
      ...
      stats: {
        ...
        netPnlSol:     +history.reduce((s, t) => s + (t.netPnlSol ?? t.pnlSol ?? 0), 0).toFixed(5),
      },
    };
    // Sign so the registry can prove this heartbeat is from the agent it claims to be.
    if (keypair && address) {
      const { _signSwarmBody } = require('./circuit');
      body = _signSwarmBody(keypair, 'heartbeat', body);
    }
...
async function buildStatus(api, wallet, cfg) {
  ...
  // Fetch prices via circuit-price-feed (Redis/gRPC → DexScreener cached server-side).
  // No direct DexScreener calls from the agent process.
  const priceFeedBase = `${api.baseUrl}/api/price-feed`;
  let priceMap = {};
  if (mints.length) {
    try {
      const url  = `${priceFeedBase}/prices?mints=${mints.join(',')}`;
      const resp = await fetch(url, { signal: AbortSignal.timeout(5_000) });
      if (resp.ok) {
        const data = await resp.json();
        for (const [mint, r] of Object.entries(data.results ?? {})) {
          if (r?.priceSol > 0) priceMap[mint] = { priceNative: r.priceSol, usdPrice: r.priceUsd ?? null };
        }
      }
    } catch { /* fall through to x402 */ }
  ...
  for (const [mint, pos] of Object.entries(held)) {
    ...
    // Use priceNative (SOL/token) only — falling back to usdPrice would produce
    // ~64x inflated P&L readings for USDC-quoted pairs (same bug monitor.js guards against).
    const currentPrice = pd?.priceNative ?? null;
    let pnlPct = null;
  ...
  if (circuitBal != null) lines.push(`• CIRCUIT: ${Math.round(circuitBal).toLocaleString()}`);
  ...
  if (ctx?.sol) {
    ...
    if (ctx.fearGreed) lines.push(`• F&G: ${ctx.fearGreed.value} — ${ctx.fearGreed.label ?? ctx.fearGreed.classification ?? ''}`);
...
function watchForResponse(messageId, telegramBot, chatId, timeoutMs = 90_000) {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      // Unlink only after delivery is confirmed — if sendMessage throws, the file
      // stays so the next watchForResponse poll can retry or it can be inspected.
      await telegramBot.api?.sendMessage(chatId, `⚠️ *Exception Response*\n\n${data.message}`, { parse_mode: 'Markdown' })
...
function start(cfg, agentCtx, telegramBot = null) {
  ...
  if (intervalMs === 0) {
    log('info', 'Heartbeat disabled (intervalMs=0)');
    return;
  }
  ...
  // Initialize Phase 2 sub-routines
  ...
  const beat = async () => {
    ...
    try {
      ...
      // 4. Registry heartbeat (simple POST, no LLM) — signed with the wallet key
      reportToRegistry(apiBase, internalK, agentId, address, wallet?.keypair);

      // 5. Phase 2 sub-routines: DCA and Ecosystem monitoring (non-blocking)
      try {
        await dcaExecutor.checkAndExecute(cfg, agentCtx, telegramBot);
      } catch (err) {
        log('error', `DCA executor error: ${err.message}`);
      }

      try {
        await ecosystemIntel.checkHealth(cfg, agentCtx, telegramBot);
      } catch (err) {
        log('error', `Ecosystem intel error: ${err.message}`);
      }
