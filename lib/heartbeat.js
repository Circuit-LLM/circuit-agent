...
async function buildStatus(api, wallet, cfg) {
  ...
  // Fetch prices for heartbeat display — use free DexScreener first, x402 as fallback.
  // DexScreener is the same source the monitor uses every 10s, so it's already warm.
  // Avoids spending CIRCUIT on status display (was: 12 paid calls/hour per position).
