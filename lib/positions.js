...
function openPosition(mint, data) {
  ...
  positions[mint] = {
    ...
    // The reasoning round that opened this position. null for autonomous opens
    // (auto-scanner, smart-money, watches) which have no LLM decision behind them.
...
// Gross basis — price move only, fees excluded. Use this for the throttles that are meant to
// react to STRATEGY quality: the circuit breaker and the re-entry cooldown.
//
// Why they must not use the net basis: the fee is ~fixed per round trip (~0.00032 SOL), so at
// the current 0.005 SOL entry it is ~4.9% of the position. That single constant flips 86% of
// trades net-negative against 75% gross, and it pushed 59% of every mint the swarm touched into
// a 24-72h ban for a "loss" the strategy did not make. The breaker then fires on three such
// trades in a row and halts buying for an hour — 223 times in three days.
//
// The result was a machine throttling itself in response to its own cost base, and reading that
// throttling back as evidence the strategy was failing. Capital-protection limits (the daily
// loss limit, survival checks) still use effPnl* — those are about real money leaving the
// wallet, where fees absolutely do count.
function grossPnlPct(t) {
  if (typeof t?.pnlPct === 'number') return t.pnlPct;
  if (t?.pnlSol != null && (t?.solSpent ?? 0) > 0) return (t.pnlSol / t.solSpent) * 100;
  return effPnlPct(t);   // last resort: a record with no gross figures at all
}
...
module.exports = {
  effPnlSol, effPnlPct, grossPnlPct, buildTag,
