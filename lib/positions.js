...
// Pass priceNative from the price-feed or DexScreener — same unit as entryPrice.
// Do NOT pass a USD price — the result would be nonsense.
// Note: monitor.js uses solSpent-based P&L (currentPrice * uiAmount − solSpent) which is
// more robust to position splits. This function is useful for simple single-price snapshots.
function calcPnlFromTokenPriceSol(pos, currentTokenPriceSol) {
  if (!pos || !currentTokenPriceSol) return null;
  const pnlPct = ((currentTokenPriceSol - pos.entryPrice) / pos.entryPrice) * 100;
...
module.exports = {
  ...
  getAll, get, count, calcPnlFromTokenPriceSol, holdMinutes,
};
