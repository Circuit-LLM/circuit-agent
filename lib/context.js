...
async function refresh(api) {
  ...
  let pricesVal = null, fearGreedVal = null, newsVal = null, swarmVal = null;
  try { pricesVal    = await api.oraclePrices(); }                       catch {}
  try { fearGreedVal = await api.fearGreed(); }                          catch {}
  ...
  // Fear & Greed — standalone on-chain index (SOL momentum + CPMM activity + DEX TVL)
  // Stored as { value, label } object to match the shape used by buildContextBlock.
  let fearGreed = null;
  if (fearGreedVal?.value != null) {
    fearGreed = { value: fearGreedVal.value, label: fearGreedVal.label ?? null };
  }
