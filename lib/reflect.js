...
async function runReflect(cfg, agentCtx, bot) {
  ...
  // 1c. Skill usage log upkeep. Grading itself is pull-based (the dashboard calls
  //     getSummary()); reflect only trims the log so it can't grow without bound.
  try {
    const skillTracker = require('./memory/skill-tracker');
    const removed = skillTracker.prune();
    if (removed) log('info', `Skill usage log pruned`, { removed });
  } catch (e) {
    log('warn', `Skill tracker prune: ${e.message}`);
  }
