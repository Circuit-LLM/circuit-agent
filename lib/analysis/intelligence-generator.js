...
/**
 * Submit recommendations for operator approval.
 * Called by reflect.js after generating report.
 * @param {array} recommendations — from _generateRecommendations
 * @returns {array} submitted recommendation IDs
 */
function submitForApproval(recommendations) {
  try {
    const approvalQueue = require('./approval-queue');
    const ids = [];
    recommendations.forEach(rec => {
      const id = approvalQueue.submitForApproval({
        title: rec.title,
        action: rec.action,
        priority: rec.priority.toUpperCase(),
        expectedLift: rec.expectedLift,
        type: _getRecType(rec.title),
        data: rec,
      });
      if (id) ids.push(id);
    });
    ...
    console.warn(`[INTEL] Failed to submit for approval: ${err.message}`);
    return [];
  }
}

function _getRecType(title) {
  if (title.includes('pattern')) return 'pattern-bias';
  if (title.includes('gate')) return 'gate-threshold';
  if (title.includes('cost')) return 'cost-optimization';
  if (title.includes('regime')) return 'regime-strategy';
  return 'general';
}
