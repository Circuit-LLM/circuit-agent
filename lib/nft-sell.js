...
class NftSellExecutor {
  ...
  async sellIntoBid(mint, { bidState, minSol } = {}) {
    ...
    if (!bidAcct) throw new Error('NFT sell: bid no longer exists');
    const bid = mkt.getBidStateDecoder().decode(bidAcct.data);

    const currency   = optVal(bid.currency);
    const cosigner   = optVal(bid.cosigner);
    const privateTk  = optVal(bid.privateTaker);
    ...
    if (currency)  throw new Error(`NFT sell: bid pays an SPL currency (${currency.slice(0, 8)}…), not native SOL — unsupported`);
    if (cosigner)  throw new Error('NFT sell: bid requires a Tensor cosigner — a self-custody seller cannot fill it');
    if (privateTk && privateTk !== this.pubkey.toBase58()) throw new Error('NFT sell: bid is private to another seller');
    if (BigInt(bid.filledQuantity ?? 0) >= BigInt(bid.quantity ?? 1)) throw new Error('NFT sell: bid already fully filled');
