...
async function processMessage(msgPath, agentCtx) {
  ...
  try {
    ...
    const model     = settings.llm?.model    ?? 'x-ai/grok-4.1-fast';
    ...
    // Increment retry counter. After MAX_RETRIES failures, move to dead-letter
    // so one permanently-broken message can't block the entire queue indefinitely.
    try {
      if (fs.existsSync(procPath)) {
        ...
        if (!parseOk) {
          // File is unparseable — move straight to dead-letter without overwriting bytes,
          ...
          if (retries >= MAX_RETRIES) {
            ...
            msgData._lastError = err.message;
            fs.writeFileSync(procPath, JSON.stringify(msgData, null, 2));
            fs.renameSync(procPath, deadPath);
            log('warn', `Message moved to dead-letter after ${retries} failures`, { file: path.basename(procPath) });
          } else {
            msgData._retries = retries;
            fs.writeFileSync(procPath, JSON.stringify(msgData, null, 2));
