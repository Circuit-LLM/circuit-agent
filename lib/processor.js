...
async function processMessage(msgPath, agentCtx) {
  ...
  try {
    ...
    // A deprecated or gone model (404), out-of-credits (402), or auth failure (401)
    // will never succeed on retry. Retrying them is what caused the 5GB log spam.
    const msg = err.message || '';
    const isPermanent = msg.includes('401') || msg.includes('402') ||
                        msg.includes('404') || msg.includes('deprecated') ||
