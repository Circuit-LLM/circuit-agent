...
async function processMessage(msgPath, agentCtx) {
  ...
  try {
    ...
    //
    // decisionId marks one reasoning round. Skills loaded in a round and positions
    // opened in that same round share it, which is what lets skill-tracker attribute
    // a trade to the skills that were actually in context when it was decided.
    const toolCtx = {
      senderId: senderId ?? sender,
      decisionId: require('crypto').randomUUID(),
      source: channel,
      api, wallet, swap, positions,
      cfg: agentCtx.cfg,
      _buyExecutedThisRound: false,
    };
