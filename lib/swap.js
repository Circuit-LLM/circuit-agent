...
class SwapExecutor {
  ...
  async _fetchActualSolDelta(signature) {
    try {
      ...
      return { deltaLamports: rawDelta + this.jitoTipLamports + txFee, feeLamports: txFee };
    } catch {
      return null;
    }
  }
  ...
  async _executeOrderFast(order, { allowParallel = false, solLamports = null } = {}) {
    ...
    let jitoTxBytes;
    try {
      jitoTxBytes = await this._buildJitoTx(order.transaction, tipLamports);
    } catch (err) {
  ...
  async buy(mint, solAmount) {
    ...
    const outTokensStr = String(result.outputAmount ?? order.outAmount ?? '0');
  ...
  async sell(mint, tokenAmount, pct = 1) {
    ...
    const result = await this._executeOrderFast(order, { allowParallel: true, solLamports: Number(order.outAmount ?? 0) }); // parallel: fastest exit
    ...
    if (result._via === 'jito' && result.signature) {
      const meta = await this._fetchActualSolDelta(result.signature);
