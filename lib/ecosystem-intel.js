...
// ── Get latest health (for dashboard/API) ──────────────────────────────────

/**
 * Get the latest health snapshot, remapped for API consumption.
 * Returns null if no health data available (fresh install or feature disabled).
 * @returns { score, tps, validators, avgPriorityFee, anomalies, breakdown } or null
 */
function getLatestHealth() {
  try {
    ...
    const alerts = _loadAlerts();
    const anomalyTypes = (alerts?.recent ?? []).map(a => a.type);

    return {
      score: current.score,
      tps: stats.recentTps,
      validators: stats.validatorCount,
      avgPriorityFee: stats.avgPriorityFee,
      breakdown: current.breakdown ?? [],
      anomalies: anomalyTypes,
    };
  } catch (err) {
    log('warn', 'getLatestHealth() error', { error: err.message });
    return null;
  }
}

module.exports = { checkHealth, initialize, getLatestHealth };
